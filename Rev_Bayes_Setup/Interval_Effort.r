...
load(paste(data_for_R_folder,"Gradstein_2020_Augmented.RData",sep="")); # Timescale Database
load(paste(data_for_R_folder,"Rock_Unit_Database.RData",sep="")); # Rock Unit Database

rock_database <- rock_unit_data$rock_unit_database;

pbdb_sites <- pbdb_data_list$pbdb_sites_refined;
nsites <- nrow(pbdb_sites);
#max(pbdb_sites$modified[!is.na(pbdb_sites$modified)])
#pbdb_sites$modified[nsites]
...
study_onset <- "Hirnantian";
study_end <- "Hirnantian";
study_onset_ma <- time_scale$ma_lb[time_scale$interval %in% study_onset];
study_end_ma <- time_scale$ma_ub[time_scale$interval %in% study_end];

study_sites_strict <- pbdb_sites[pbdb_sites$ma_lb<=study_onset_ma & pbdb_sites$ma_ub>=study_end_ma,];
formation_nos <- sort(unique(study_sites_strict$formation_no[study_sites_strict$formation_no>0]));
rock_nos <- sort(unique(study_sites_strict$rock_no_sr[study_sites_strict$rock_no_sr>0]));
nform <- length(formation_nos);
nrock <- length(rock_nos);
names(formation_nos) <- rock_database$formation[match(formation_nos,rock_database$rock_no)];
names(rock_nos) <- rock_database$full_name[match(rock_nos,rock_database$rock_no)];
formation_counts <- hist(match(study_sites_strict$formation_no[study_sites_strict$formation_no>0],formation_nos),
						 breaks=0:nform,plot=FALSE)$counts;
names(formation_counts) <- names(formation_nos);
formation_counts <- sort(formation_counts,decreasing=TRUE);
occurrence_counts_by_formation_spc <- occurrence_counts_by_formation <- vector(length=length(formation_counts));
names(occurrence_counts_by_formation_spc) <- names(occurrence_counts_by_formation) <- names(formation_counts);
for (oc in 1:length(formation_counts))	{
	rn <- formation_nos[match(names(formation_counts[oc]),names(formation_nos))]
	formation_finds <- pbdb_finds[pbdb_finds$collection_no %in% study_sites_strict$collection_no[study_sites_strict$formation_no %in% rn],];
	occurrence_counts_by_formation[oc] <- nrow(formation_finds);
	occurrence_counts_by_formation_spc[oc] <- sum(formation_finds$accepted_rank %in% c("species","subspecies"));
	}

rock_unit_counts <- hist(match(study_sites_strict$rock_no_sr[study_sites_strict$rock_no_sr>0],rock_nos),
												 breaks=0:nrock,plot=FALSE)$counts;
names(rock_unit_counts) <- names(rock_nos);
rock_unit_counts <- sort(rock_unit_counts,decreasing=TRUE);
occurrence_counts_by_rock_unit_spc <- occurrence_counts_by_rock_unit <- vector(length=length(rock_unit_counts));
names(occurrence_counts_by_rock_unit_spc) <- names(occurrence_counts_by_rock_unit) <- names(rock_unit_counts);
for (oc in 1:length(rock_unit_counts))	{
	rn <- rock_nos[match(names(rock_unit_counts[oc]),names(rock_nos))];
	rock_unit_finds <- pbdb_finds[pbdb_finds$collection_no %in% study_sites_strict$collection_no[study_sites_strict$rock_no_sr %in% rn],];
	occurrence_counts_by_rock_unit[oc] <- nrow(rock_unit_finds);
	occurrence_counts_by_rock_unit_spc[oc] <- sum(rock_unit_finds$accepted_rank %in% c("species","subspecies"));
	}
rock_unit_summary <- data.frame(rock_unit=names(rock_unit_counts),sites=rock_unit_counts,nfinds=occurrence_counts_by_rock_unit,nfinds_sp=occurrence_counts_by_rock_unit_spc)
rock_unit_summary <- rock_unit_summary[order(-rock_unit_summary$sites,-rock_unit_summary$nfinds,-rock_unit_summary$nfinds_sp),];

paste(pbdb_sites$collection_no[pbdb_sites$rock_no_sr %in% 3],collapse=",")

study_taxon_rank <- pbdb_taxonomy$accepted_rank[match(study_taxon,pbdb_taxonomy$taxon_name)]
study_sites <- pbdb_sites[pbdb_sites$ma_lb>study_end_ma & pbdb_sites$ma_ub<study_onset_ma,];
