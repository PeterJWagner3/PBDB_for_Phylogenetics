...
# count collections occupied by subintervals, with possible fractional counts
tally_collections_occupied_by_subinterval <- function(taxon_collections,hierarchical_chronostrat,constrain=FALSE,temporal_precision=0.1)	{
# taxon_collections: collection number
# early_interval: earliest possible chronostratigraphic interval for corresponding coll_no
# hierarchical_chronostrat: table denoting which chronostratigraphic units are subunits of others
# constrain: return only relevant time intervals; otherwise, return results for entire time scale.
...
# written 2026-09-02
# pbdb_collections <- pbdb_sites_fuzz;
# collection_no <- pbdb_collections$collection_no[1];
tally_sites_occupied_by_collection_no <- function(collection_no,pbdb_collections,finest_chronostrat)	{
prob_in_bin <- vector(length=nrow(finest_chronostrat));
names(prob_in_bin) <- finest_chronostrat$interval;
cn <- match(collection_no,pbdb_collections$collection_no);
bin_span <- pbdb_collections$bin_lb[cn]:pbdb_collections$bin_ub[cn];
bspan <- length(bin_span);
age_range <- abs(pbdb_collections$ma_lb[cn]-pbdb_collections$ma_ub[cn]);
if (bspan==1)	{
	prob_in_bin[bin_span] <- 1;
	} else if (bspan>1)	{
	prob_in_bin[bin_span[1]] <- abs(pbdb_collections$ma_lb[cn]-finest_chronostrat$ma_ub[bin_span[1]])/age_range;
	prob_in_bin[bin_span[bspan]] <- abs(finest_chronostrat$ma_lb[bin_span[bspan]]-pbdb_collections$ma_ub[cn])/age_range;
	mb <- 2;
	while (mb<(bspan))	{
		prob_in_bin[bin_span[mb]] <- abs(finest_chronostrat$ma_lb[bin_span[mb]]-finest_chronostrat$ma_ub[bin_span[mb]])/age_range
		mb <- mb+1;
		}
	}
return(prob_in_bin);
...
# written 2026-09-02
tally_sites_occupied_by_collection_no_sapply <- function(pbdb_collections,finest_chronostrat,update=TRUE)	{
collection_no <- pbdb_collections$collection_no;
if (update)	{
	prob_sites_in_bin <- base::t(pbapply::pbsapply(collection_no,tally_sites_occupied_by_collection_no,pbdb_collections,finest_chronostrat));
	} else	{
	prob_sites_in_bin <- base::t(sapply(collection_no,tally_sites_occupied_by_collection_no,pbdb_collections,finest_chronostrat));
	}
return(colSums(prob_sites_in_bin));
}
...
r_c <- 0;
rsites <- nrow(relevant_collections);
nbins <- nrow(finest_chronostrat);
prob_find_this_set <- array(0,dim=c(rsites,nbins));
while (r_c < nrow(relevant_collections))	{
	r_c <- r_c+1;
	prob_find_this_case <- array(0,dim=c(length(finest_chronostrat$interval)));
	aa <- min(finest_chronostrat$bin_first[match(relevant_collections$interval_lb[r_c],finest_chronostrat$interval)]);
	...
	prob_find_this_set[r_c,] <- prob_find_this_case;
#	prob_find_this_set <- rbind(prob_find_this_set,prob_find_this_case);
	}
...
# written 2026-01-16
accersi_taxon_no_fa_and_la_dates <- function(taxon_no,paleodb_finds,pbdb_sites,taxon_rank="species")	{
# modified 2025-10-20
#taxon_col <- match(taxon_rank,colnames(paleodb_finds));
if (taxon_rank %in% c("species","subspecies"))	{
	taxon_finds <- paleodb_finds[paleodb_finds$accepted_no %in% taxon_no,];
	} else	{
	taxon_col_no <- paste(taxon_rank,"_no",sep="");
	taxon_finds <- paleodb_finds[paleodb_finds[,taxon_col_no] %in% taxon_no,];
	taxon_sites <- pbdb_sites[pbdb_sites$collection_no %in% taxon_finds$collection_no,];
	}
output_fa_la <- data.frame(fa_lb=0,fa_ub=0,la_lb=0,la_ub=0);
if (nrow(taxon_finds)>0)	{
	output_fa_la$fa_lb <- max(abs(taxon_sites$ma_lb));
	output_fa_la$fa_ub <- max(abs(taxon_sites$ma_ub));
	output_fa_la$la_lb <- min(abs(taxon_sites$ma_lb));
	output_fa_la$la_ub <- min(abs(taxon_sites$ma_ub));
	}
return(output_fa_la)
}
