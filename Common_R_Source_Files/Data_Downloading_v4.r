...
bad_pbdb_collections <- c(271,293,443,1315,1583,2237,3832,10285,24174,24585,24829,26826,27240,28188,42505,42506,42507,42508,42509,49496,55455,98219,154013,160725,214129,226273);
hell_no <- F;
bad_taxon_entries <- c("n. gen.  n. sp. sp.","UNKNOWN"," sp.", "cf.  sp.");
not_a_species <- c("sp.","indet.");
reided <- c("R","RI","RF","RIF");

protected_rock_names <- c("Flat Top","Top Springs","Top Hosie Shale","Top Terrigenous Series","Round Top","Little Hat Top","Top Hosie Limestone","Top Hosie","Top Hut","Top Hut Limestone","Upper Tiouririne");
...
freshwater_environments <- c("levee","mire/swamp","fluvial-lacustrine indet.","fluvial-deltaic indet.","lacustrine - large","lacustrine - small","pond","crater lake","lacustrine delta plain","lacustrine interdistributary bay","lacustrine delta front","lacustrine prodelta","lacustrine deltaic indet.","lacustrine indet.","mire/swamp","spring");
...
marine_only_phyla <- c("Brachiopoda","Chaetognatha","Ctenophora","Echinodermata","Echiura","Hemichordata","Lophotrochozoa","Lorcifera","Petalonamae","Phoronida","Priapulida","Proarticulata","Sarcomastigophora","Sipuncula","Vetulicolia");
marine_only_classes <- c("Anthozoa","Archaeocyatha","Artiopoda","Ascidiacea","Calcarea","Cephalopoda","Cephalaspidomorphi","Chondrichthyes","Conodonta","Cribricyatha","Fusulinata","Galeaspidomorphi","Heteractinida","Hexactinellida","Irregulares","Paragastropoda","Placodermi","Polyplacophora","Regulares","Scaphopoda","Sclerospongiae","Stromatoporoidea","Tergomya","Trilobita");
marine_only_orders <- c("Agelasida","Ancorinida","Astrophorida","Axinellida","Chaetetida","Chondrosida","Clavulina","Craniellida","Dendroceratida","Dictyoceratida","Guadalupiida","Hadromerida","Halichondrida","Halisarcida","Haplosclerida","Lithistida","Megalithistida","Monalithistida","Operculospongia","Pachastrellida","Permosphincta","Plakinida","Poecilosclerida","Protomonaxonida","Sigmatosclerophorida","Spirophorida","Spirosclerophorida","Streptosclerophorida","Tabulospongida","Tetralithistida","Vaceletida","Verongida","Verticillitida");
nonmarine_classes <- c("Chilopoda","Diplopoda");

ichnofossil_labels <- c("I","IF","VF","VIF","VI");

direct_date_methods <- c("K-Ar","14C (calibrated)","Sr isotope","fission track","paleomagnetic","U/Th","Ar/Ar","U/Pb","14C","ESR","Rb-Sr","astronomical");
direct_date_fields <- c("direct_ma_value","direct_ma_error","direct_ma_unit","direct_ma_method","max_ma_value","max_ma_error","max_ma_unit","max_ma_method","min_ma_value","min_ma_error","min_ma_unit","min_ma_method");
direct_date_numerical_fields <- c("direct_ma_value","direct_ma_error","max_ma_value","max_ma_error","min_ma_value","min_ma_error");
...
paleodb_numeric_fields <- c("no","ma","size","occs","colls","geoplate","plate","subset","lat","lng","paleolat","paleolng","bin","max_ma","min_ma","n_colls","n_occs","gplate_no","splate_no","pubyr","direct_ma_value","direct_ma_error","max_ma_value","max_ma_error","min_ma_value","min_ma_error");
...
algae_phyla <- c("Chlorophyta","Dinophyta","Dinoflagellata","Glaucophyta","Haptophyta","Heterokontophyta","Rhodophyta");
fungi_phyla <- c("Ascomycota","Basidiomycota","Chytridiomycota","Oomycota");
land_plants <- land_plant_phyla <- c("Angiospermae","Coniferophyta","Ginkgophyta","Haptophyta","Pinophyta","Psilophytophyta","Pteridophyta","Spermatophyta","Tracheophyta","Pteridospermophyta","Lycopodiophyta","Bryophyta","Gymnosperm","Equisetophyta","Cycadophyta","Peltaspermophyta","Lycophyta");
single_celled_phyla <- c("Acoela","Actinopoda","Bacillariophyta","Cercozoa","Ciliophora","Cyanobacteria","Foraminifera","Heterokontophyta","Ochrophyta","Prymnesiophyta","Prasinophyta","Radiolaria","Sarcodina");
...
for (cn in 1:ncol(paleodb_data))	{
	...
	if((sum(cnm %in% paleodb_numeric_fields) > 0) && !colnames(paleodb_data)[cn] %in% c("collection_size","direct_ma_method","direct_ma_unit","max_ma_method","max_ma_unit","min_ma_method","min_ma_unit"))	{
...
#		return(ISOdate(entry_dates[3],entry_dates[2],entry_dates[1],entry_times[1],entry_times[2],entry_times[3]));
	...
	return(as.Date(as.numeric(pbdb_date),origin="1899-12-30"));
	} else {
	print(pbdb_date);
	}
}
...
accersi_occurrence_data <- function(taxa="Life",onset="Archean",end="Holocene",basic_environments="terr,marine,unknown",species_only=TRUE,clean_entered_taxa=TRUE,ecospace=F,directory="",save_files=TRUE,output_type=".csv") {
# Arguments:
...
accersi_collection_data_from_dataframe <- function(collections,standardize_members=FALSE) {
if (!is.null(collections$formation.1))	{
	collections$formation.1 <- NULL;
	collections$member.1 <- NULL;
	collections$geological_group.1 <- NULL;
...
# written 2026-03-07: use "absolute" dates to restrict oldest and/or youngest dates of collections
# paleodb_sites <- all_sites;
restrict_site_ages_with_direct_dates <- function(paleodb_sites,direct_date_methods=c("K-Ar","14C (calibrated)","Sr isotope","fission track","paleomagnetic","U/Th","Ar/Ar","U/Pb","14C","ESR","Rb-Sr","astronomical"))	{
if (is.null(paleodb_sites$ma_lb))	{
	colnames(paleodb_sites)[colnames(paleodb_sites)=="max_ma"] <- "ma_lb";
	colnames(paleodb_sites)[colnames(paleodb_sites)=="min_ma"] <- "ma_ub";
	}
paleodb_sites$max_ma_value[is.na(paleodb_sites$max_ma_value)] <- 0;
paleodb_sites$max_ma_error[is.na(paleodb_sites$max_ma_error)] <- 0;
paleodb_sites$min_ma_value[is.na(paleodb_sites$min_ma_value)] <- 0;
paleodb_sites$min_ma_error[is.na(paleodb_sites$min_ma_error)] <- 0;
paleodb_sites$direct_ma_value[is.na(paleodb_sites$direct_ma_value)] <- 0;
paleodb_sites$direct_ma_error[is.na(paleodb_sites$direct_ma_error)] <- 0;

psites <- nrow(paleodb_sites);
ypb_d <- (1:psites)[paleodb_sites$direct_ma_unit %in% "YBP"];
ypb_l <- (1:psites)[paleodb_sites$max_ma_unit %in% "YBP"];
ypb_u <- (1:psites)[paleodb_sites$min_ma_unit %in% "YBP"];
paleodb_sites[ypb_d,c("direct_ma_value","direct_ma_error")] <- paleodb_sites[ypb_d,c("direct_ma_value","direct_ma_error")]/10^6;
paleodb_sites[ypb_l,c("max_ma_value","max_ma_error")] <- paleodb_sites[ypb_l,c("max_ma_value","max_ma_error")]/10^6;
paleodb_sites[ypb_u,c("min_ma_value","min_ma_error")] <- paleodb_sites[ypb_u,c("min_ma_value","min_ma_error")]/10^6;
ka_d <- (1:psites)[paleodb_sites$direct_ma_unit %in% "Ka"];
ka_l <- (1:psites)[paleodb_sites$max_ma_unit %in% "Ka"];
ka_u <- (1:psites)[paleodb_sites$min_ma_unit %in% "Ka"];
paleodb_sites[ka_d,c("direct_ma_value","direct_ma_error")] <- paleodb_sites[ka_d,c("direct_ma_value","direct_ma_error")]/10^3;
paleodb_sites[ka_l,c("max_ma_value","max_ma_error")] <- paleodb_sites[ka_l,c("max_ma_value","max_ma_error")]/10^3;
paleodb_sites[ka_u,c("min_ma_value","min_ma_error")] <- paleodb_sites[ka_u,c("min_ma_value","min_ma_error")]/10^3;
paleodb_sites$direct_ma_unit[paleodb_sites$direct_ma_unit %in% c("YBP","Ka")] <- paleodb_sites$max_ma_unit[paleodb_sites$max_ma_unit %in% c("YBP","Ka")] <- paleodb_sites$min_ma_unit[paleodb_sites$min_ma_unit %in% c("YBP","Ka")] <- "Ma";
paleodb_sites[,c("direct_ma_value","direct_ma_unit","max_ma_value","max_ma_unit","min_ma_value","min_ma_unit")]
errors <- c(paleodb_sites$min_ma_error[paleodb_sites$min_ma_value>0 & paleodb_sites$min_ma_error>0],paleodb_sites$max_ma_error[paleodb_sites$max_ma_value>0 & paleodb_sites$max_ma_error>0],paleodb_sites$direct_ma_error[paleodb_sites$direct_ma_value>0 & paleodb_sites$direct_ma_error>0]);
dates <- c(paleodb_sites$min_ma_value[paleodb_sites$min_ma_value>0 & paleodb_sites$min_ma_error>0],paleodb_sites$max_ma_value[paleodb_sites$max_ma_value>0 & paleodb_sites$max_ma_error>0],paleodb_sites$direct_ma_value[paleodb_sites$direct_ma_value>0 & paleodb_sites$direct_ma_error>0]);
median_error <- median(errors/dates);

nsites <- nrow(paleodb_sites);
lower_bounds <- (1:nsites)[paleodb_sites$max_ma_value>0 & paleodb_sites$max_ma_method %in% direct_date_methods];
lost_lower_errors <- lower_bounds[paleodb_sites$max_ma_error[lower_bounds]==0];
paleodb_sites$max_ma_error[lost_lower_errors] <- median_error*paleodb_sites$max_ma_value[lost_lower_errors];
upper_bounds <- (1:nsites)[paleodb_sites$min_ma_value>0 & paleodb_sites$min_ma_method %in% direct_date_methods];
lost_upper_errors <- upper_bounds[paleodb_sites$min_ma_error[upper_bounds]==0];
paleodb_sites$min_ma_error[lost_upper_errors] <- median_error*paleodb_sites$min_ma_value[lost_upper_errors];
direct_dates <- (1:nsites)[paleodb_sites$direct_ma_value>0 & paleodb_sites$direct_ma_method %in% direct_date_methods];
lost_direct_errors <- direct_dates[paleodb_sites$direct_ma_error[direct_dates]==0];
paleodb_sites$direct_ma_error[lost_direct_errors] <- median_error*paleodb_sites$direct_ma_value[lost_direct_errors]

double_bounded <- lower_bounds[lower_bounds %in% upper_bounds];
flip_flop <- double_bounded[(paleodb_sites$min_ma_value[double_bounded]-paleodb_sites$min_ma_error[double_bounded])>(paleodb_sites$max_ma_value[double_bounded]+paleodb_sites$max_ma_error[double_bounded])];
if (length(flip_flop)>0)	{
	xx <- paleodb_sites$max_ma_value[flip_flop];
	yy <- paleodb_sites$max_ma_error[flip_flop];
	aa <- paleodb_sites$max_ma_unit[flip_flop];
	bb <- paleodb_sites$max_ma_method[flip_flop];
	paleodb_sites$max_ma_value[flip_flop] <- paleodb_sites$min_ma_value[flip_flop];
	paleodb_sites$max_ma_error[flip_flop] <- paleodb_sites$min_ma_error[flip_flop];
	paleodb_sites$max_ma_unit[flip_flop] <- paleodb_sites$min_ma_unit[flip_flop];
	paleodb_sites$max_ma_method[flip_flop] <- paleodb_sites$min_ma_method[flip_flop];
	paleodb_sites$min_ma_value[flip_flop] <- xx;
	paleodb_sites$min_ma_error[flip_flop] <- yy;
	paleodb_sites$min_ma_unit[flip_flop] <- aa;
	paleodb_sites$min_ma_method[flip_flop] <- bb;
	}

lower_bounds <- lower_bounds[!lower_bounds %in% direct_dates];
upper_bounds <- upper_bounds[!upper_bounds %in% direct_dates];
aa <- cbind(paleodb_sites$ma_lb[lower_bounds],paleodb_sites$max_ma_value[lower_bounds]+paleodb_sites$max_ma_error[lower_bounds]);
zz <- cbind(paleodb_sites$ma_ub[upper_bounds],paleodb_sites$min_ma_value[upper_bounds]-paleodb_sites$min_ma_error[upper_bounds]);

paleodb_sites$ma_lb[lower_bounds] <- rowMins(aa);
paleodb_sites$ma_ub[upper_bounds] <- rowMaxs(zz);
paleodb_sites$ma_lb[direct_dates] <- paleodb_sites$direct_ma_value[direct_dates]+paleodb_sites$direct_ma_error[direct_dates];
paleodb_sites$ma_ub[direct_dates] <- paleodb_sites$direct_ma_value[direct_dates]-paleodb_sites$direct_ma_error[direct_dates];

# find collections where ma_lb is more recent than the minimum direct date
too_young <- upper_bounds[paleodb_sites$ma_lb[upper_bounds]<=(paleodb_sites$min_ma_value[upper_bounds]-paleodb_sites$min_ma_error[upper_bounds])];
#too_young <- lower_bound[pbdb_sites_refined$ma_lb[lower_bound]<(pbdb_sites_refined$max_ma_value[lower_bound]-pbdb_sites_refined$max_ma_error[lower_bound])];
too_young <- too_young[!is.na(too_young)];
ty <- 0;
while (ty < length(too_young))	{
	ty <- ty+1;
	t_y <- too_young[ty];
	if (is.na(paleodb_sites$max_ma_error[t_y]))	{
		paleodb_sites$max_ma_error[t_y] <- 0;
		}
	if (paleodb_sites$max_ma_error[t_y]>0 & paleodb_sites$max_ma_error[t_y]==0)	{
		xx <- median_error*paleodb_sites$max_ma_value[t_y];
		paleodb_sites$max_ma_error[t_y] <- round(xx/(0.5*10^floor(log10(xx))),0)*(0.5*10^floor(log10(xx)));
		}
	if (paleodb_sites$min_ma_error[t_y]==0)	{
		xx <- median_error*paleodb_sites$min_ma_value[t_y];
		paleodb_sites$min_ma_error[t_y] <- round(xx/(0.5*10^floor(log10(xx))),0)*(0.5*10^floor(log10(xx)));
		}
	paleodb_sites[t_y,c("collection_no","zone","ma_lb","ma_ub","max_ma_value","max_ma_error","min_ma_value","min_ma_error","direct_ma_value","direct_ma_error")];
	# routine if we have both upper and lower bounds
	ub_lb <- paleodb_sites$min_ma_value[t_y]+paleodb_sites$min_ma_error[t_y];
	ub_ub <- paleodb_sites$min_ma_value[t_y]-paleodb_sites$min_ma_error[t_y];
	if (paleodb_sites$max_ma_value[t_y]>0) {
		lb_lb <- paleodb_sites$max_ma_value[t_y]+paleodb_sites$max_ma_error[t_y];
		lb_ub <- paleodb_sites$max_ma_value[t_y]-paleodb_sites$max_ma_error[t_y];
		if (lb_ub>paleodb_sites$ma_lb[t_y] & lb_ub>ub_lb & paleodb_sites$ma_lb[t_y]!=paleodb_sites$ma_ub[t_y])	{
			paleodb_sites$ma_lb[t_y] <- lb_ub;
			paleodb_sites$ma_ub[t_y] <- ub_lb;
			} else	{
			paleodb_sites$ma_lb[t_y] <- lb_lb;
			paleodb_sites$ma_ub[t_y] <- ub_lb;
			}
		}  else	{
		paleodb_sites$ma_lb[t_y] <- ub_lb;
		paleodb_sites$ma_ub[t_y] <- ub_ub;
#		pbdb_sites_refined$ma_ub[t_y] <- min(c(pbdb_sites_refined$ma_ub[t_y],(pbdb_sites_refined$max_ma_value[t_y]-pbdb_sites_refined$max_ma_error[t_y])));
		}
	paleodb_sites[t_y,c("collection_no","zone","ma_lb","ma_ub","max_ma_value","max_ma_error","min_ma_value","min_ma_error","direct_ma_value","direct_ma_error")];
#	if (pbdb_sites_refined$zone[t_y]!="") <- "mis-zoned";
	if (paleodb_sites$ma_lb[t_y]==paleodb_sites$ma_ub[t_y])	{
		if (paleodb_sites$min_ma_value[t_y]<paleodb_sites$ma_ub[t_y])	{
			paleodb_sites$ma_ub[t_y] <- paleodb_sites$min_ma_value[t_y];
			} else	{
			paleodb_sites$ma_lb[t_y] <- paleodb_sites$ma_lb[t_y]+paleodb_sites$min_ma_value[t_y]/200;
			paleodb_sites$ma_ub[t_y] <- paleodb_sites$ma_ub[t_y]-paleodb_sites$min_ma_value[t_y]/200;
			}
		} else if (paleodb_sites$ma_ub[t_y] < (paleodb_sites$min_ma_value[t_y]-paleodb_sites$min_ma_error[t_y]))	{
		paleodb_sites$ma_ub[t_y] <- (paleodb_sites$min_ma_value[t_y]-paleodb_sites$min_ma_error[t_y])
		}
	paleodb_sites[t_y,c("collection_no","zone","ma_lb","ma_ub","max_ma_value","max_ma_error","min_ma_value","min_ma_error","direct_ma_value","direct_ma_error")];
	}

too_old <- lower_bounds[paleodb_sites$ma_ub[lower_bounds]>=(paleodb_sites$max_ma_value[lower_bounds]+paleodb_sites$max_ma_error[lower_bounds])];
#too_old <- upper_bound[pbdb_sites_refined$ma_ub[upper_bound]>(pbdb_sites_refined$min_ma_value[upper_bound]-pbdb_sites_refined$min_ma_error[upper_bound])];
too_old <- too_old[!is.na(too_old)];
to <- 0;
while (to < length(too_old))	{
	to <- to+1;
	t_o <- too_old[to];
	if (paleodb_sites$max_ma_value[t_o]>0 & paleodb_sites$max_ma_error[t_o]==0)	{
		xx <- median_error*paleodb_sites$max_ma_value[t_o];
		paleodb_sites$max_ma_error[t_o] <- round(xx/(0.5*10^floor(log10(xx))),0)*(0.5*10^floor(log10(xx)));
		}
	if (paleodb_sites$min_ma_value[t_o]>0 & paleodb_sites$min_ma_error[t_o]==0)	{
		xx <- median_error*paleodb_sites$min_ma_value[t_o];
		paleodb_sites$min_ma_error[t_o] <- round(xx/(0.5*10^floor(log10(xx))),0)*(0.5*10^floor(log10(xx)));
		}
	paleodb_sites[t_o,c("collection_no","ma_lb","ma_ub","max_ma_value","max_ma_error","min_ma_value","min_ma_error","min_ma_method","direct_ma_value","direct_ma_error")]

	lb_lb <- paleodb_sites$max_ma_value[t_o]+paleodb_sites$max_ma_error[t_o];
	lb_ub <- paleodb_sites$max_ma_value[t_o]-paleodb_sites$max_ma_error[t_o];

	if (paleodb_sites$min_ma_value[t_o]>0) {
		ub_lb <- paleodb_sites$min_ma_value[t_o]+paleodb_sites$min_ma_error[t_o];
		ub_ub <- paleodb_sites$min_ma_value[t_o]-paleodb_sites$min_ma_error[t_o];
		if (ub_lb<paleodb_sites$ma_lb[t_o] & lb_ub>ub_lb & paleodb_sites$ma_lb[t_o]!=paleodb_sites$ma_ub[t_o])	{
			paleodb_sites$ma_lb[t_o] <- lb_ub;
			paleodb_sites$ma_ub[t_o] <- ub_lb;
			} else	{
			paleodb_sites$ma_lb[t_o] <- lb_lb;
			paleodb_sites$ma_ub[t_o] <- ub_lb;
			}
		}  else	{
		paleodb_sites$ma_lb[t_o] <- lb_lb;
		paleodb_sites$ma_ub[t_o] <- lb_ub;
#		pbdb_sites_refined$ma_ub[t_o] <- min(c(pbdb_sites_refined$ma_ub[t_o],(pbdb_sites_refined$max_ma_value[t_o]-pbdb_sites_refined$max_ma_error[t_o])));
		}
	paleodb_sites[t_o,c("collection_no","ma_lb","ma_ub","max_ma_value","max_ma_error","min_ma_value","min_ma_error","min_ma_method","direct_ma_value","direct_ma_error")]
	}

db_ties <- double_bounded[paleodb_sites$ma_lb[double_bounded]==paleodb_sites$ma_ub[double_bounded]];
paleodb_sites$ma_lb[db_ties] <- paleodb_sites$max_ma_value[db_ties]+paleodb_sites$max_ma_error[db_ties];
paleodb_sites$ma_ub[db_ties] <- paleodb_sites$min_ma_value[db_ties]-paleodb_sites$min_ma_error[db_ties];

direct_date_mishaps <- direct_dates[paleodb_sites$ma_lb[direct_dates]<=paleodb_sites$ma_ub[direct_dates]];
paleodb_sites$ma_lb[direct_date_mishaps] <- paleodb_sites$direct_ma_value[direct_date_mishaps]+abs(paleodb_sites$direct_ma_error[direct_date_mishaps]);
paleodb_sites$ma_ub[direct_date_mishaps] <- paleodb_sites$direct_ma_value[direct_date_mishaps]-abs(paleodb_sites$direct_ma_error[direct_date_mishaps]);

impossible_dates <- (1:nsites)[paleodb_sites$ma_ub>paleodb_sites$ma_lb];
id <- 0;
#ddd <- c("collection_no","max_ma","min_ma","ma_lb","ma_ub","direct_ma_value","max_ma_value","max_ma_error","min_ma_value","min_ma_error");
while (id < length(impossible_dates))	{
	id <- id+1;
	idc <- impossible_dates[id];
#	pbdb_sites_refined[idc,c("collection_no","ma_lb","ma_ub","max_ma_value","max_ma_error","min_ma_value","min_ma_error","min_ma_method","direct_ma_value","direct_ma_error")]
	if (idc %in% double_bounded)	{
		paleodb_sites$ma_lb[idc] <- paleodb_sites$max_ma_value[idc]+paleodb_sites$max_ma_error[idc]
		paleodb_sites$ma_ub[idc] <- paleodb_sites$min_ma_value[idc]-paleodb_sites$min_ma_error[idc]
		}
#	pbdb_sites_refined[idc,c("collection_no","ma_lb","ma_ub","max_ma_value","max_ma_error","min_ma_value","min_ma_error","min_ma_method","direct_ma_value","direct_ma_error")]
	}

if (is.null(paleodb_sites$max_ma))	{
	colnames(paleodb_sites)[colnames(paleodb_sites)=="ma_lb"] <- "max_ma";
	colnames(paleodb_sites)[colnames(paleodb_sites)=="ma_ub"] <- "min_ma";
	}

return(paleodb_sites);
}
...
# added 2026-03-07
remove_duplicate_sites_paleodb <- function(paleodb_sites)	{
nsites <- nrow(paleodb_sites);
site_counts <- hist(match(paleodb_sites$collection_no,unique(paleodb_sites$collection_no)),breaks=0:nsites,plot=FALSE)$counts;
names(site_counts) <- unique(paleodb_sites$collection_no);
duplicates <- site_counts[site_counts>1];
dp <- 0;
while (dp < length(duplicates))	{
	dp <- dp+1;
	asites <- (1:nsites)[paleodb_sites$collection_no == as.numeric(names(duplicates[dp]))];
	dup_sites <- paleodb_sites[asites,];
	rownames(dup_sites) <- asites;
	# first, look for most recently modified
	dup_sites <- dup_sites[dup_sites$modified %in% max(dup_sites$modified),]
	# next, keep ones with direct dates
	if (nrow(dup_sites)>1 & sum(dup_sites[,direct_date_numerical_fields])>0)
		dup_sites <- dup_sites[rowSums(dup_sites[,direct_date_numerical_fields])>0,]
	# next, keep ones with zones
	if (nrow(dup_sites)>1 & sum(dup_sites$zone!="")>0)
		dup_sites <- dup_sites[dup_sites$zone!="",];
	# next, keep ones with second rock unit information
	if (nrow(dup_sites)>1 & !is.null(dup_sites$formation_alt))
		if (max(rowSums(dup_sites[,c("formation_alt","member_alt","geological_group_alt")]!=""))>0)
			dup_sites <- dup_sites[rowSums(dup_sites[,c("formation_alt","member_alt","geological_group_alt")]!="")==max(rowSums(dup_sites[,c("formation_alt","member_alt","geological_group_alt")]!="")),]
	cut_sites <- asites[!asites %in% as.numeric(rownames(dup_sites))];
	paleodb_sites <- paleodb_sites[!rownames(paleodb_sites) %in% cut_sites,];
	}
return(paleodb_sites);
}
...
#	httpT <- paste("https://paleobiodb.org/data1.2/taxa/list.csv?base_name=",taxon,"                                         &show=full,app,immparent,classext,etbasis,seq,img,ref,ent,entname,crmod",sep="");
	httpT <- paste("https://paleobiodb.org/data1.2/taxa/list.csv?base_name=",taxon,"&variant=all&rel=all_children&",interval,"&show=full,attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,etbasis,pres,ref,refattr,seq,img,ent,entname,crmod",sep="");
	} else if (inc_parents)	{
	httpT <- paste("https://paleobiodb.org/data1.2/taxa/list.csv?base_name=",taxon,"&variant=all&rel=all_parents&",interval,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,etbasis,pres,ref,refattr,seq,img,ent,entname,crmod",sep="");
	} else	{
#	httpT <- paste("https://paleobiodb.org/data1.2/taxa/list.csv?match_name=",taxon,"&show=attr,parent,class,classext,refattr,crmod",sep="");
	httpT <- paste("https://paleobiodb.org/data1.2/taxa/list.csv?match_name=",taxon,"&variant=all",                   interval,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,etbasis,pres,ref,refattr,seq,img,ent,entname,crmod",sep="");
	}
...
#junior="Otarion hollandi"
restore_junior_synonyms <- function(junior,pbdb_finds,pbdb_taxonomy)	{
pbdb_finds$accepted_no[pbdb_finds$identified_no %in% pbdb_taxonomy$taxon_no[pbdb_taxonomy$orig_no %in%  pbdb_taxonomy$orig_no[match(junior,pbdb_taxonomy$taxon_name)]]] <- pbdb_taxonomy$taxon_no[match(junior,pbdb_taxonomy$taxon_name)];
pbdb_finds$accepted_name[pbdb_finds$identified_no %in% pbdb_taxonomy$taxon_no[pbdb_taxonomy$orig_no %in%  pbdb_taxonomy$orig_no[match(junior,pbdb_taxonomy$taxon_name)]]] <- junior;
return(pbdb_finds);
}
...
find_species_type_localities <- function(species_name,pbdb_finds,pbdb_sites)	{
coll_finds <- pbdb_finds$collection_no[pbdb_finds$accepted_name %in% species_name]
type_localities <- coll_finds[gsub("n. sp.","",pbdb_finds$identified_name[pbdb_finds$accepted_name %in% species_name])!=pbdb_finds$identified_name[pbdb_finds$accepted_name %in% species_name]];
if (length(type_localities)>1)	{
		type_finds <- pbdb_finds[pbdb_finds$collection_no %in% type_localities & pbdb_finds$accepted_name %in% species_name,];
		epithets <- c(divido_species_epithets(species_name),
									divido_species_epithets(spay_or_neuter_your_species(species_name)));
		type_localities <- type_finds$collection_no[gsub(epithets[1],"",type_finds$identified_name)!=type_finds$identified_name];
		}
if (length(type_localities)>0)	{
	type_localities <- type_localities[1];
	} else {
	type_localities <- 0;
	}
return(type_localities);
}
...
surnames <- ref_authors <- sort(unique(c(pbdb_references$author1last,pbdb_references$author2last[pbdb_references$author2last!=""])));
#surnames <- romanize_authors(ref_authors);
...
if (named_rock_unit=="" || named_rock_unit %in% protected_rock_names)	{
...
#	named_rock_unit <- str_ucfirst(named_rock_unit);		# function no longer works
	...
	#	named_rock_unit <- gsub("'s ", "s ",named_rock_unit);
		...
		named_rock_unit <- gsub("top of ","",named_rock_unit);
		named_rock_unit <- gsub("bottom of ","",named_rock_unit);
		named_rock_unit <- gsub("middle of ","",named_rock_unit);
...
#		print(named_rock_unit);
	...
	if (delete_informal)	{
		informals <- c("basal","base","inferieur","lower","lowermost","lowest","middle","upper","uppermost","superieur","informal","tongue","equivalent","unter","unterer","ober","oberer");
		n_r_u <- strsplit(named_rock_unit," ")[[1]];
		rock_names <- length(n_r_u);
...
#find_higher_taxon_for_taxon_no(taxon_no=167450,higher_rank="kingdom",pbdb_taxonomy)
...
##### Redate PBDB collections with Wagner Database ######
redate_refined_paleodb_collections_with_rock_database <- function(pbdb_sites_refined,rock_database,time_scale,zone_database)	{

}

#rock_no <- 3419;
redate_particular_rock_based_on_rock_no <- function(rock_no,pbdb_sites_refined,rock_database,time_scale,zone_database)	{
these_sites <- pbdb_sites_refined[pbdb_sites_refined$rock_no_sr %in% rock_no,];
this_rock <- rock_database[rock_database$rock_no==rock_no,];
these_sites$ma_lb>this_rock$ma_lb
these_sites$ma_lb<=this_rock$ma_ub
pbdb_sites_refined$rock_no[pbdb_sites_refined$collection_no==24172]
pbdb_sites_refined$ma_lb[pbdb_sites_refined$collection_no==190701]
pbdb_sites_refined$ma_ub[pbdb_sites_refined$collection_no==190701]
}
...
##### Match PBDB Rock Data to Wagner Rock Database ######
