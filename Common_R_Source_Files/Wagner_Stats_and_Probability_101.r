...
colProds <- function(dmat,use_logs=T)	{
if (nrow(dmat)==1) return(dmat);
rps <- c();
if (use_logs)	{
	lndmat <- log(dmat);
	rps <- exp(colSums(lndmat));
	} else	{
	for (i in 1:nrow(dmat))
		rps <- c(rps,prod(dmat[,i]));
	}
...
if (!is.null(rownames(data_matrix)))	names(row_medians) <- rownames(data_matrix);
return(row_medians)
}
...
rowVars <- function(m)	{
row_variances <- vector(length=nrow(m));
for (i in 1:nrow(m))	row_variances[i] <- var(simplify2array(m[i,]));
return(row_variances);
}

colVars <- function(m)	{
col_variances <- vector(length=ncol(m));
for (i in 1:ncol(m))	col_variances[i] <- var(m[,i]);
ncol(col_variances);
}

colcumsum <- function(m,descending=T)	{
cumulative_m <- m;
mm <- nrow(m);
for (i in 1:ncol(m))	{
	if (descending)	{
		cumulative_m[,i] <- cumsum(m[,i]);
		} else	{
		cumulative_m[,i] <- cumsum(m[mm:1,i]);
		}
	}
return(cumulative_m);
}

rowcumsum <- function(m,descending=T)	{
cumulative_m <- m;
mm <- ncol(m);
for (i in 1:nrow(m))	{
	if (descending)	{
		...
		cumulative_m[i,] <- cumsum(m[i,mm:1])[mm:1];
		}
	}
return(cumulative_m);
}

rowsort <- function(m,descending=F)	{
for (i in 1:nrow(m))	m[i,] <- sort(m[i,],descending=descending);
return(m);
}
...
slope_lsq <- function(x, y) cov(x, y) / var(x);

intercept_lsq <- function(x, y) return(mean(y) - slope_lsq(x, y) * mean(x));

slope_rma <- function(x, y) return(sqrt(var(y)) / sqrt(var(x)));

intercept_rma <- function(x, y) return(mean(y) - slope_rma(x, y) * mean(x));

rma_error_bars <- function(x,y,bootstraps=500)	{
nspec <- length(x);
output <- data.frame(slope=rep(0,bootstraps),intercept=rep(0,bootstraps));
for (bt in 1:bootstraps)	{
	boot <- ceiling(runif(nspec)*nspec);
	output$slope[bt] <- slope_rma(x[boot],y[boot])
	output$intercept[bt] <- intercept_rma(x[boot],y[boot])
	}
return(output);
}
...
#aics <- aics[1,]
akaikes_weights <- function(aics)	{
delta_aics <- aics-min(aics);
return(exp(-0.5 *delta_aics)/sum(exp(-0.5 *delta_aics)));
}
