...
# use dated beds to limit maximum & minimum onsets & ends of rocks within sections
constrain_rock_ages_in_section_with_dated_beds_strict <- function(this_section_info,radiometric_dates)	{
#this_section_info <- constrain_rock_ages_in_section_basic(this_section_info);
srocks <- nrow(this_section_info);
hot_rocks <- this_section_info$rock_no[this_section_info$rock_no %in% c(radiometric_dates$rock_no_sr,radiometric_dates$formation_no)];
hot_rock_info <- radiometric_dates[radiometric_dates$rock_no_sr %in% hot_rocks,];
wallflowers <- this_section_info$rock_no[!this_section_info$rock_no %in% hot_rocks];
hrocks <- length(hot_rocks);
sr <- hr <- 0;
# look to see if any "median" radiometric dates are out of order
section_test <- this_section_info[this_section_info$rock_no %in% hot_rocks,];
if (hrocks>1)	{
#	hr <- 0;
	for (hr in 1:hrocks)	{
#		hr <- hr+1;
		this_hot_rock <- hot_rock_info[hot_rock_info$rock_no_sr %in% hot_rocks[hr],];
		section_test$onset_ub[hr] <- max(this_hot_rock$ma_mean);
		section_test$end_lb[hr] <- min(this_hot_rock$ma_mean);
		}
	# make sure rocks in in right order
	while (sum(abs(order(-section_test$onset_ub)-(1:hrocks)))>0)	{
		section_test_2 <- constrain_rock_ages_in_section_with_dated_beds(this_section_info=section_test,radiometric_dates = radiometric_dates);
		section_test <- this_section_info[this_section_info$rock_no %in% hot_rocks,];
		hot_rock_info$spans <- abs(hot_rock_info$ma_lb_event_bed-hot_rock_info$ma_ub_event_bed)
		xx <- match(max(hot_rock_info$spans),hot_rock_info$spans);
		if (length(xx)<nrow(hot_rock_info))	{
			hot_rock_info <- hot_rock_info[!(1:nrow(hot_rock_info)) %in% xx,];
			section_test <- section_test[section_test$rock_no %in% hot_rock_info$rock_no_sr,];
			for (hr in 1:length(unique(section_test$rock_no)))	{
				this_hot_rock <- hot_rock_info[hot_rock_info$rock_no_sr %in% section_test$rock_no[hr],];
				section_test$onset_ub[hr] <- max(this_hot_rock$ma_mean);
				section_test$end_lb[hr] <- min(this_hot_rock$ma_mean);
				}
			hot_rocks <- unique(section_test$rock_no);
			hrocks <- length(hot_rocks);
			hot_rock_info <- hot_rock_info[hot_rock_info$rock_no_sr %in% hot_rocks,];
			}
		}
	}
# start here. It gets rid of the terrible date, but doesn't fix the dates with the good ones.
while (sr < srocks && hr < hrocks)	{
	sr <- sr+1;
	while (!this_section_info$rock_no[sr] %in% hot_rocks)	sr <- sr+1;
	lo_rocks <- hi_rocks <- c();
...
#		if (nrow(rock_dates)==0)	which(radiometric_dates==srock_no,arr.ind=T)
		colnames(rock_dates) <- gsub("_event_bed","",colnames(rock_dates));
		rock_dates <- rock_dates[rock_dates$ma_mean<this_section_info$onset_lb[sr] & rock_dates$ma_mean>this_section_info$end_ub[sr],];
		if (nrow(rock_dates)==0)	break;
		rock_dates$rock_no_sr <- srock_no;
#		orig_date_span <- data.frame(fa_lb=max(this_section_info$onset_lb[sr]),fa_ub=max(this_section_info$onset_ub[sr]),la_lb=min(this_section_info$end_lb[sr]),la_ub=min(this_section_info$end_ub[sr]));
		orig_date_span <- this_section_info[sr,];
		rock_date_span <- data.frame(fa_lb=max(rock_dates$ma_mean),fa_ub=max(rock_dates$ma_mean),la_lb=min(rock_dates$ma_mean),la_ub=min(rock_dates$ma_mean));
		...
		if (nrow(rock_dates)>0)	{
			if (sr < srocks && this_section_info$rock_no[sr+1] %in% hot_rocks)	{
				...
				colnames(higher_hot_rocks) <- gsub("_event_bed","",colnames(higher_hot_rocks));
				higher_hot_rocks$rock_no_sr <- hrock_no;
				higher_date_span <- data.frame(fa_lb=max(higher_hot_rocks$ma_mean),fa_ub=max(higher_hot_rocks$ma_mean),la_lb=min(higher_hot_rocks$ma_mean),la_ub=min(higher_hot_rocks$ma_mean));
				overlapping_dates <- unique(rbind(rock_dates[rock_dates$ma_lb<=higher_date_span$fa_lb & rock_dates$ma_ub>=higher_date_span$la_ub,],
																					higher_hot_rocks[higher_hot_rocks$ma_lb<=rock_date_span$fa_lb & higher_hot_rocks$ma_ub>=rock_date_span$la_ub,]));
				# WORK ON THIS 2026-07-17 ###
				if (nrow(overlapping_dates)>0)	{
					if (srock_no %in% overlapping_dates$rock_no_sr && hrock_no %in% overlapping_dates$rock_no_sr)	{
						this_section_info$end_lb[sr] <- max(min(overlapping_dates$ma_lb[overlapping_dates$rock_no_sr %in% srock_no]),
																								max(overlapping_dates$ma_lb[overlapping_dates$rock_no_sr %in% hrock_no]));
...
accersi_age_bounds_on_other_rocks_in_section_with_rock_no <- function(rock_no,section,rock_database,radiometric_dates,rock_to_zone_database,rock_to_isotope_excursion,precision=0.1,dbug=FALSE,fuzzy=TRUE)	{
...
if (hrocks>0)	{
	if (fuzzy)	{
		section_info_radi <- constrain_rock_ages_in_section_with_dated_beds(this_section_info=section_info,radiometric_dates);
		}	else	{
		section_info_radi <- constrain_rock_ages_in_section_with_dated_beds_strict(this_section_info=section_info,radiometric_dates);
		}
	}
...
# added 2026-07-21
get_chronostratigraphic_boundaries <- function(relv_sites)	{
unique_bounds <- unique(relv_sites[,c("ma_lb","ma_ub")]);
unique_bounds <- unique_bounds[order(-unique_bounds$ma_lb,-unique_bounds$ma_ub),];
unique_dates <- sort(unique(c(unique_bounds$ma_lb,unique_bounds$ma_ub)),decreasing=TRUE);
unique_lbs <- sort(unique(unique_bounds$ma_lb),decreasing=TRUE);
unique_ubs <- sort(unique(unique_bounds$ma_ub),decreasing=TRUE);
boundary_sets <- data.frame(ma_lb=unique_bounds$ma_lb[1],ma_ub=unique_bounds$ma_ub[1]);
def_boundaries <- unique_lbs[unique_lbs %in% unique_ubs];
ub <- 1;
while (ub<nrow(unique_bounds))	{
	ub <- ub+1;
	bu <- nrow(boundary_sets);
	if (!do_two_ranges_overlap(lb_a=boundary_sets$ma_lb[bu],ub_a=boundary_sets$ma_ub[bu],lb_b=unique_bounds$ma_lb[ub],ub_b=unique_bounds$ma_ub[ub]))	{
		
#		} else	{
		boundary_sets <- rbind(boundary_sets,unique_bounds[ub,]);
		}
	}	

#ma_lb_unq <- unique(unique_bounds$ma_lb[order(-unique_bounds$ma_lb)])
#ma_ub_unq <- unique(unique_bounds$ma_ub[order(-unique_bounds$ma_ub)])
#boundaries <- ma_ub_unq[ma_ub_unq %in% ma_lb_unq];
#nboundaries <- length(boundaries)
#boundary_sets <- data.frame(ma_lb=rep(0,nboundaries+1),ma_ub=rep(0,nboundaries+1));
#boundary_sets$ma_lb[1] <- max(ma_lb_unq);
#boundary_sets$ma_ub[nboundaries+1] <- min(ma_ub_unq);
#nb <- 0
#while (nb<nboundaries)	{
#	nb <- nb+1;
#	boundary_sets$ma_ub[nb] <- boundary_sets$ma_lb[nb+1] <- boundaries[nb];
#	}
