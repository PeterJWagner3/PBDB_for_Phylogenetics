...
#	end_pt <- (1:length(nexus))[tolower(nexus) %in% c("end;"," end;")]
	while (!tolower(nexus[tx_pt]) %in% c("end;"," end;","endblock;"," endblock;"))	{
#		yyy <- paste(nexusfile[tx_pt,], collapse = "");
...
# turn taxon name into something usable in Nexus files. This allows for cf., aff., etc.
nexify_otu_name <- function(otu_name)	{
if (gsub("\\(","",otu_name)!=otu_name | gsub("\\.","",otu_name)!=otu_name | gsub("\\?","",otu_name)!=otu_name)	{
	otu_name <- paste("'",otu_name,"'",sep="")
	} else	{
