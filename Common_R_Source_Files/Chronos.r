...
# lump overly short stage slices together
#intervals_to_lump <- c("Ka1","Ka2");
#time_scale_to_condense <- finest_chronostrat;
#new_interval_name <- "Ka1-2";
lump_two_plus_intervals <- function(time_scale_to_condense,intervals_to_lump,new_interval_name=NULL)  {
# keep working on this!
condensed_scale <- time_scale_to_condense[!time_scale_to_condense$interval %in% intervals_to_lump,];
lumped_scale <- time_scale_to_condense[time_scale_to_condense$interval %in% intervals_to_lump,];
lumped_interval <- lumped_scale[1,];
lumped_interval$ma_lb <- max(lumped_scale$ma_lb);
lumped_interval$ma_ub <- min(lumped_scale$ma_ub);
lumped_interval$basal_biozone <- lumped_scale$basal_biozone[match(lumped_interval$ma_lb,lumped_scale$ma_lb)];
lumped_interval$basal_biozone_scale <- lumped_scale$basal_biozone_scale[match(lumped_interval$ma_lb,lumped_scale$ma_lb)];
lumped_interval$basal_biozone_zone_no <- lumped_scale$basal_biozone_zone_no[match(lumped_interval$ma_lb,lumped_scale$ma_lb)];
lumped_interval$basal_zone_modifier <- lumped_scale$basal_zone_modifier[match(lumped_interval$ma_lb,lumped_scale$ma_lb)];
lumped_interval$terminal_biozone <- lumped_scale$terminal_biozone[match(lumped_interval$ma_ub,lumped_scale$ma_ub)];
lumped_interval$terminal_biozone_scale <- lumped_scale$terminal_biozone_scale[match(lumped_interval$ma_ub,lumped_scale$ma_ub)];
lumped_interval$terminal_biozone_zone_no <- lumped_scale$terminal_biozone_zone_no[match(lumped_interval$ma_ub,lumped_scale$ma_ub)];
lumped_interval$terminal_zone_modifier <- lumped_scale$terminal_zone_modifier[match(lumped_interval$ma_ub,lumped_scale$ma_ub)];
if (is.null(new_interval_name))
	new_interval_name <- paste(intervals_to_lump[1],intervals_to_lump[length(intervals_to_lump)],sep="-")
lumped_interval$interval <- new_interval_name;
condensed_scale <- rbind(condensed_scale,lumped_interval);
if (condensed_scale$ma_lb[1]>condensed_scale$ma_lb[2])	{
	condensed_scale <- condensed_scale[order(-condensed_scale$ma_lb),];
	} else	{
	condensed_scale <- condensed_scale[order(condensed_scale$ma_lb),];
	}
return(condensed_scale);
