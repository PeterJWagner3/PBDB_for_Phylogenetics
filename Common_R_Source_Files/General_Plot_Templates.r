...
sysfonts::font_add("Franklin Gothic Medium", regular = "Franklin Gothic Medium.ttf");
sysfonts::font_add("Franklin Gothic Medium Italic", regular = "Franklin Gothic Medium Italic.ttf");
sysfonts::font_add("Futura", regular = "Futura.ttc");
sysfonts::font_add("Times New Roman",regular = "Times New Roman.ttf");
sysfonts::font_add("Times New Roman Italic",regular = "Times New Roman Italic.ttf");
sysfonts::font_add("Arial",regular = "Arial.ttf");
sysfonts::font_add("Arial Bold",regular = "Arial Bold.ttf");
sysfonts::font_add("Helvetica",regular = "Helvetica.ttc");
sysfonts::font_add("Baskerville",regular = "Baskerville.ttc");
sysfonts::font_add("Georgia",regular = "Georgia.ttf");
sysfonts::font_add("Myriad Pro",regular = "MyriadPro-Regular.ttf");
sysfonts::font_add("Myriad Pro Bold",regular = "MyriadPro-Bold.ttf");
sysfonts::font_add("Myriad Pro Italic",regular = "MyriadPro-It.ttf");

franky <- "Franklin Gothic Medium";
franky_italic <- "Franklin Gothic Medium Italic";
shades <- "Futura";
whimey <- "Times New Roman";
whimey_italci <- "Times New Roman Italic";
sprite <- "Arial";
bold_sprite <- "Arial Bold";
hellvent <- "Helvetica";
hound <- "Baskerville";
find_me_votes <- "Georgia";
myriad <- "Myriad Pro";
myriad_bold <- "Myriad Pro Bold";
myriad_italic <- "Myriad Pro Italic";

halloween_colors <- c("orange","purple2","forestgreen","deepskyblue4","firebrick","gold");
yule_colors <- c("green2","red2","purple","white","cyan","hotpink");
basic_rainbow <- c("red","orange","yellow","green","cyan","dodgerblue","blue","purple");
lower_words <- c("The","Of","In","A","An","Mm","Cm");

#### General Graphic Stuff ####
par(pin=c(4,4));
ysize_time <- default_minor_time_axis_size <- 4.285714285;
phanerozoic<- c("Ediacaran","Cambrian","Ordovician","Silurian","Devonian","Carboniferous","Permian","Triassic","Jurassic","Cretaceous","Paleogene","Neogene","Quaternary");
paleozoic<- c("Ediacaran","Cambrian","Ordovician","Silurian","Devonian","Carboniferous","Permian");
teal <- "#008080";
mauve <- "#E0AFFF";
...
# written 2026-03-17 for E6;
plot_overlapping_variables <- function(joint_data,variable_colors,dash_length=1,lwd=1)	{
colnames(joint_data) <- c("x","y");
nd <- nrow(joint_data);
nv <- length(variable_colors);
xspan <- joint_data$x[1]-joint_data$x[nd];
if (xspan>0)	dash_length <- -abs(dash_length);
xbreaks <- c(seq(joint_data$x[1],joint_data$x[nd]+dash_length,by=dash_length),joint_data$x[nd]);
if (xspan>0)	{
	xbreaks <- xbreaks[xbreaks>=joint_data$x[nd]];
	} else	{
	xbreaks <- xbreaks[xbreaks<=joint_data$x[nd]];
	}
for (b in 1:length(xbreaks))	{
#	b <- b+1;
	x1 <- xbreaks[b];
	if ((b+1) < length(xbreaks))	{
		x2 <- xbreaks[b+1]
		} else	{
		x2 <- xbreaks[length(xbreaks)];
		}
	bin1 <- sum(joint_data$x<=x1);
	bin2 <- sum(joint_data$x<=x2);
	xx <- unique(c(x1,joint_data$x[(bin1+1):bin2],x2));
	xx <- xx[xx<=x2 & xx>=x1];

	var_no <- b %% nv;
	if (var_no==0)	var_no <- nv;
	if (length(xx)==2)	{
		slope <- (joint_data$y[bin1+1]-joint_data$y[bin1])/abs(joint_data$x[bin1+1]-joint_data$x[bin1]);
		y1 <- joint_data$y[bin1]+slope*abs(x1-joint_data$x[bin1]);
		y2 <- joint_data$y[bin1]+slope*abs(x2-joint_data$x[bin1]);
		lines(-abs(c(x1,x2)),c(y1,y2),lwd=lwd,col=variable_colors[var_no]);
		} else	{
		slope_a <- (joint_data$y[bin1+1]-joint_data$y[bin1])/abs(joint_data$x[bin1+1]-joint_data$x[bin1]);
		yy <- c(joint_data$y[bin1]+slope_a*abs(xx[1]-joint_data$x[bin1]));
		for (bb in 2:(length(xx)-1))	{
			bbb <- (bin1:bin2)[bb];
			yy <- c(yy,joint_data$y[bbb])
			}
		slope_b <- (joint_data$y[bin2+1]-joint_data$y[bin2])/abs(joint_data$x[bin2+1]-joint_data$x[bin2]);
		yy <- c(yy,joint_data$y[bin2]+slope_b*abs(xx[length(xx)]-joint_data$x[bin2]));
		lines(xx,yy,lwd=3,col=variable_colors[var_no]);
		}
	}
}
...
accersi_appropriate_stratigraphic_intervals_for_plots <- function(start,finish,time_scale)	{
start <- abs(start);
finish <- abs(finish);
if ((start-finish)>200)	{
	time_scale$chronostratigraphic_rank[time_scale$interval=="Quaternary"] <- "Period";
	standard_time_scale <- time_scale[time_scale$chronostratigraphic_rank=="Period",]
	} else	{
	standard_time_scale <- time_scale[time_scale$scale=="International" & time_scale$chronostratigraphic_rank=="Stage",]
	}
standard_time_scale <- standard_time_scale[!is.na(standard_time_scale$interval),];
standard_time_scale <- standard_time_scale[standard_time_scale$ma_ub<start,];
standard_time_scale <- standard_time_scale[standard_time_scale$ma_lb>finish,];

standard_time_scale$ma_lb <- -abs(standard_time_scale$ma_lb);
standard_time_scale$ma_ub <- -abs(standard_time_scale$ma_ub);
standard_time_scale <- standard_time_scale[order(standard_time_scale$ma_lb),];	# Make sure that time scale is correct order, with oldest first
return(standard_time_scale)
}

# ord <- mny
draw_time_scale <- function(time_scale_to_plot,onset,end,strat_colors="",strat_names="",line_color="black",ord,height,strat_label_size,axis=1)	{
if (onset[1]<0 && time_scale_to_plot[1]>0)
	time_scale_to_plot <- -1*time_scale_to_plot;
first_period <- sum(abs(time_scale_to_plot)>=abs(onset));
last_period <- sum(abs(time_scale_to_plot)>=abs(end));
if (length(strat_colors)==0)	strat_colors <- rep("white",last_period-first_period);

for (b in first_period:last_period)	{
#	rect(max(onset,time_scale_to_plot[b]),ord,min(end,time_scale_to_plot[b+1]),height,col=as.character(strat_colors[b]),border=as.character(strat_colors[b]))
...
for (s in first_period:last_period)	segments(time_scale_to_plot[s],ord,time_scale_to_plot[s],height,lwd=0.5,col=line_color)
# put in decisive final line
if (!is.na(match(end,time_scale_to_plot)))	segments(end,ord,end,height,col=line_color);
...
if (length(strat_names)>0)	{
	...
	for (i in first_period:last_period)	{
		mid <- c(mid,(max(onset,time_scale_to_plot[i])+min(end,time_scale_to_plot[(i+1)]))/2)
...
for (i in 1:length(ages))	ages[i] <- 100*(st-i)
...
Phanerozoic_Timescale_Plot_Flexible <- function(onset=-541,end=0,time_scale_to_plot,mxy,mny,
												use_strat_labels=TRUE,strat_names,strat_colors,
												plot_title="",ordinate="",abscissa="Ma",yearbreaks,
												xsize=6, ysize=4.285714285,hues=TRUE,colored="base",
												alt_back=FALSE,alt_back_hue="gray90",strat_label_size=1,
												title_off=-1,xlab_off=2,ylab_off=2,xlab_size=1,
												ylab_size=1,title_size=1.25,myr_size=1,font="")	{
# UPDATED 2016-12-31
...
# xlab_off: offset of x-axis label ("Ma")
# ylab_off: offset of y-axis label
# xlab_size: size of x-axis label ("Ma")
# myr_size: size of dates
#	ylab_size: size of y-axis label
...
exy <- (mxy-mny)/25;
dxy <- (4.285714285/ysize)*exy;
par(pin=c(xsize,ysize));
plot(NA,type='n',axes=FALSE,main=plot_title,xlab="",ylab="",xlim=c(onset,end),ylim=c(mny-exy,mxy));
if (font!="")	{
	if (plot_title!="")
		title(main=plot_title, line=title_off, cex.main=title_size,family=font);
	title(xlab=abscissa, line=xlab_off, cex.lab=xlab_size,family=font);
	title(ylab=ordinate, line=ylab_off, cex.lab=ylab_size,family=font);
	} else	{
	if (plot_title!="")
		title(main=plot_title, line=title_off, cex.main=title_size);
	title(xlab=abscissa, line=xlab_off, cex.lab=xlab_size);
	title(ylab=ordinate, line=ylab_off, cex.lab=ylab_size);
	}
...
yearbreaks <- as.numeric(sort(yearbreaks));
for (i in 1:draws)	{
	...
	if (i < draws)	{
		...
		if (font=="")	{
			axis(1,at=tcs,tcl=pts[i],labels=abs(tcs),lwd=0.0,lwd.ticks=4/3,cex.axis=myr_size);
			} else	{
			axis(1,at=tcs,tcl=pts[i],labels=abs(tcs),lwd=0.0,lwd.ticks=4/3,cex.axis=myr_size,family=font);
			}
		}
	}
...
if (hues)	{
	if (colored=="base")	{
		for (b in first_period:last_period)
			rect(max(onset,time_scale_to_plot[b]),mny+(2*dxy)-(2*exy),min(end,time_scale_to_plot[b+1]),(mny-2*exy),col=as.character(strat_colors[b]),border=as.character(strat_colors[b]),lwd=0);
		} else {
		for (b in first_period:last_period)
...
for (s in first_period:last_period)	segments(time_scale_to_plot[s],mny+(2*dxy)-(2*exy),time_scale_to_plot[s],mny-2*exy,lwd=0.5,col="gray25")
# put in decisive final line
if (!is.na(match(end,time_scale_to_plot)))	segments(end,mny+(2*dxy)-(2*exy),end,mny-2*exy);
segments(onset,mny+(2*dxy)-(2*exy),end,mny+(2*dxy)-(2*exy),lwd=2)
...
#	for (i in 1:(lst-1))		dummy[i] <- ((mny-2*exy)+(mny+2*dxy-2*exy))/2;
	dummy <- rep(((mny-2*exy)+(mny+2*dxy-2*exy))/2,lst-1);
	#add something
	font_size <- min(2,3*thin_bin*xsize*strat_label_size);
	font_size <- max(0.5,font_size);
...
#	print(font_size);
	if (font=="")	{
		...
		text(mid[first_period:last_period],dummy[first_period:last_period],label=strat_names[first_period:last_period],cex=font_size,family=font);
		}
	}
}

Phanerozoic_Timescale_Plot_Hierarchical <- function(onset=-541,end=0,time_scale_to_plot_minor,time_scale_to_plot_major,mxy,mny,use_strat_labels=TRUE,strat_names_minor,strat_names_major,strat_colors_minor,strat_colors_major,strat_font_colors_minor="#000000",strat_font_colors_major="#000000",plot_title="",ordinate="",abscissa="Ma",yearbreaks,xsize=6,ysize=4.285714285,hues=TRUE,colored="base",alt_back=FALSE,alt_back_hue="gray90",strat_label_size_minor=1,strat_label_size_major=1.5,hierarchical_partition=FALSE,hier_part_lwd=0.5,hier_part_col="gray50",timescale_pos=-1,title_off=-1,xlab_off=2,ylab_off=2,title_size=1.25,xlab_size=1,ylab_size=1,myr_size=1,font="")	{
# Written for Blastoid Project 2025-04-30
#	Smaller updates 2018-08-31
# onset: when to start x-axis (use -100 for 100 Million years ago)
# end: when to end x-axis 0
# time_scale: vector giving start of each bin, with one more than bins
...
# alt_back: alternating white and light gray backgrouns
# use_strat_labels added 2016/09/23
# hierarchical_partition: line separating (say) epochs from stages
# hier_part_lwd: width line separating (say) epochs from stages
# xlab_off: offset of x-axis label ("Ma")
# ylab_off: offset of y-axis label
# xlab_size: size of x-axis label ("Ma")
# myr_size: size of dates
#	ylab_size: size of y-axis label
# font: what font to use
if (length(strat_font_colors_major)==1)	strat_font_colors_major <- rep(strat_font_colors_major[1],length(strat_colors_major));
if (length(strat_font_colors_minor)==1)	strat_font_colors_minor <- rep(strat_font_colors_minor[1],length(strat_colors_minor));
if (time_scale_to_plot_minor[1]>0)	time_scale_to_plot_minor <-	-1*time_scale_to_plot_minor;
if (onset>0)			onset <- -1*onset;
#if (end>0)				end <- -1*end;
lst <- length(time_scale_to_plot_minor)
first_period <- sum(time_scale_to_plot_minor<=onset)
last_period <- sum(time_scale_to_plot_minor<=end)
...
dxy <- (4.285714285/ysize)*exy;
par(pin=c(xsize,ysize));
plot(NA,type='n',axes=FALSE,main="",xlab="",ylab="",xlim=c(onset,end),ylim=c(mny-exy,mxy));
if (font!="")	{
	title(xlab=abscissa, line=xlab_off, cex.lab=xlab_size,family=font);
	if (length(ordinate)==1) {
		title(ylab=ordinate, line=ylab_off, cex.lab=ylab_size,family=font);
		} else if (length(ordinate)==2)	{
		title(ylab=ordinate[1], line=ylab_off+ylab_size, cex.lab=ylab_size,family=font);
		title(ylab=ordinate[2], line=ylab_off, cex.lab=ylab_size,family=font);
		}
	if (plot_title!="")
		title(main=plot_title, line=title_off, cex.main=title_size,family=font);
	} else	{
	...
	if (length(ordinate)==1) {
		...
		title(ylab=ordinate[1], line=ylab_off+ylab_size, cex.lab=ylab_size);
		title(ylab=ordinate[2], line=ylab_off, cex.lab=ylab_size);
		}
	if (plot_title!="")
		title(main=plot_title, line=title_off, cex.main=title_size);
	}

if (max(time_scale_to_plot_minor)<end)	end <- max(time_scale_to_plot_minor);
#axis(1,at=seq(onset,end,by=abs(onset-end)),tcl=0.0,labels=FALSE,lwd=2)
if (draws==2)	{
	pts <- c(-0.15,-0.30)
	}	else if (draws==3)	{
...
for (i in 1:draws)	{
	...
	if (i < draws)	{
		...
		if (font=="")	{
			axis(1,at=tcs,tcl=pts[i],labels=abs(tcs),lwd=0.0,lwd.ticks=4/3,cex.axis=myr_size,padj=timescale_pos);
			} else	{
			axis(1,at=tcs,tcl=pts[i],labels=abs(tcs),lwd=0.0,lwd.ticks=4/3,cex.axis=myr_size,padj=timescale_pos,family=font);
			}
		}
	}

if (alt_back && colored!="backdrop")	for (b in first_period:last_period)	if (gtools::even(b))	rect(max(onset,time_scale_to_plot_minor[b]),mny,min(end,time_scale_to_plot_minor[b+1]),mxy,col=alt_back_hue,border=alt_back_hue)

top_minor <- mny+(2*dxy)-(2*exy);
bottom_major <- mny-2*exy;
bottom_minor <- top_major <- mean(c(top_minor,bottom_major))

if (hues)	{
	if (colored=="base")	{
		for (b in first_period:last_period)
#			rect(max(onset,time_scale_to_plot_minor[b]),mny+(2*dxy)-(2*exy),min(end,time_scale_to_plot_minor[b+1]),(mny-2*exy),col=as.character(strat_colors_minor[b]),border=as.character(strat_colors_minor[b]),lwd=0);
			rect(max(onset,time_scale_to_plot_minor[b]),top_minor,min(end,time_scale_to_plot_minor[b+1]),bottom_minor,col=as.character(strat_colors_minor[b]),border=as.character(strat_colors_minor[b]),lwd=0);
		for (b in 1:length(strat_names_major))
				rect(max(onset,time_scale_to_plot_major[b]),top_major,min(end,time_scale_to_plot_major[b+1]),bottom_major,col=as.character(strat_colors_major[b]),border=as.character(strat_colors_major[b]),lwd=0);
		} else {
		for (b in first_period:last_period)
			rect(max(onset,time_scale_to_plot_minor[b]),mny,min(end,time_scale_to_plot_minor[b+1]),mxy,col=strat_colors_minor[b],border=strat_colors_minor[b],lwd=0);
		rect(onset,mny+exy,end,mxy,col=makeTransparent("white",50),border=makeTransparent("white",50))
		}
	}

# put in lines separating stages
for (s in first_period:last_period)	{
	if (!time_scale_to_plot_minor[s] %in% time_scale_to_plot_major)	{
		segments(time_scale_to_plot_minor[s],top_minor,time_scale_to_plot_minor[s],bottom_minor,lwd=0.5,col="gray25")
		} else	{
		segments(time_scale_to_plot_minor[s],top_minor,time_scale_to_plot_minor[s],bottom_major,lwd=1.0,col="gray13")
		}
	}
# put in decisive final line
if (!is.na(match(end,time_scale_to_plot_minor)))	segments(end,mny+(2*dxy)-(2*exy),end,mny-2*exy);
segments(onset,mny+(2*dxy)-(2*exy),end,mny+(2*dxy)-(2*exy),lwd=2)
segments(onset,mny-2*exy,end,mny-2*exy,lwd=2)
if (hierarchical_partition)
	segments(onset,bottom_minor,end,bottom_minor,lwd=hier_part_lwd,col=hier_part_col);

if (use_strat_labels)	{
	thin_bin <- min(abs(time_scale_to_plot_minor[2:length(time_scale_to_plot_minor)]-time_scale_to_plot_minor[1:(length(time_scale_to_plot_minor)-1)])/abs(onset-end));
	mid_minor <- vector(length=(lst-1));
	for (i in 1:(lst-1))		mid_minor[i] <- (max(onset,time_scale_to_plot_minor[i])+min(end,time_scale_to_plot_minor[(i+1)]))/2;
	dummy_minor <- vector(length=(lst-1));
	for (i in 1:(lst-1))		dummy_minor[i] <- mean(c(top_minor,bottom_minor));
	#add something
#	font_size <- min(2,3*thin_bin*xsize*strat_label_size_minor);
	if (font=="")	{
		for (mm in first_period:(last_period-1))
			if (strat_names_minor[mm]=="Cm")	{
				text(mid_minor[mm],dummy_minor[mm],label="C",cex=strat_label_size_minor,col=strat_font_colors_minor[mm]);
				text(mid_minor[mm],dummy_minor[mm],label="-",cex=strat_label_size_minor,col=strat_font_colors_minor[mm]);
				} else	{
				text(mid_minor[mm],dummy_minor[mm],label=strat_names_minor[mm],cex=strat_label_size_minor,col=strat_font_colors_minor[mm]);
				}
#		text(mid_minor[first_period:last_period],dummy_minor[first_period:last_period],label=strat_names_minor[first_period:last_period],cex=strat_label_size_minor,col=strat_font_colors_minor[first_period:(last_period-1)]);
		} else	{
		for (mm in first_period:(last_period-1))
			if (strat_names_minor[mm]=="Cm")	{
				text(mid_minor[mm],dummy_minor[mm],label="C",cex=strat_label_size_minor,col=strat_font_colors_minor[mm],family=font);
				text(mid_minor[mm],dummy_minor[mm],label="-",cex=strat_label_size_minor,col=strat_font_colors_minor[mm],family=font);
				} else	{
				text(mid_minor[mm],dummy_minor[mm],label=strat_names_minor[mm],cex=strat_label_size_minor,col=strat_font_colors_minor[mm],family=font);
				}
#		text(mid_minor[first_period:last_period],dummy_minor[first_period:last_period],label=strat_names_minor[first_period:last_period],cex=strat_label_size_minor,family=font,col=strat_font_colors_minor[first_period:(last_period-1)]);
		}
	mid_major <- vector(length=length(strat_names_major));
	dummy_major <- (top_major+bottom_major)/2;
	thin_bin <- min(abs(time_scale_to_plot_major[2:length(time_scale_to_plot_major)]-time_scale_to_plot_minor[1:(length(time_scale_to_plot_major)-1)])/abs(onset-end));
	font_size <- min(c(2,3*thin_bin*xsize*strat_label_size_major,strat_label_size_major));
	for (i in 1:length(strat_names_major))		mid_major[i] <- (max(onset,time_scale_to_plot_major[i])+min(end,time_scale_to_plot_major[(i+1)]))/2;
	if (font=="")	{
		text(mid_major,dummy_major,label=strat_names_major,cex=font_size,col=strat_font_colors_major);
		} else	{
		text(mid_major,dummy_major,label=strat_names_major,cex=font_size,col=strat_font_colors_major,family=font);
		}
	}
}

Phanerozoic_Timescale_Plot_Flexible_Ordinate <- function(onset=-541,end=0,time_scale_to_plot,mxx,mnx,use_strat_labels=TRUE,strat_names,strat_colors,plot_title="",ordinate="Ma",abscissa="",yearbreaks,ysize=6,xsize=4.285714285,hues=TRUE,colored="base",alt_back=FALSE,alt_back_hue="gray90",strat_label_size=1,xlab_off=2,ylab_off=4,xlab_size=1,ylab_size=1,myr_size=1,font="")	{
# UPDATED 2016-12-31
#	Smaller updates 2018-08-31
# onset: when to start x-axis (use -100 for 100 Million years ago)
# end: when to end x-axis 0
# time_scale: vector giving start of each bin, with one more than bins
# mxx: maximum value for y-axis
# mnx: minimum value for y-axis
# use_strat_labels: true means that you put labels on stratigraphic unts
# strat_names: names to use for those labels
# strat_colors
# ordinate: label for Y-axis
...
exx <- (mxx-mnx)/25;
dxx <- (4.285714285/xsize)*exx;

par(pin=c(xsize,ysize));
plot(NA,type='n',axes=FALSE,main=plot_title,xlab="",ylab="",ylim=c(onset,end),xlim=c(mnx-exx,mxx));
if (font!="")	{
	title(xlab=abscissa, line=xlab_off, cex.lab=xlab_size,family=font);
	if (length(ordinate)==1)	{
		title(ylab=ordinate, line=ylab_off, cex.lab=ylab_size,family=font);
...
if (max(time_scale_to_plot)<end)	end <- max(time_scale_to_plot);
...
for (i in 1:draws)	{
	...
	if (i < draws)	{
		...
		if (font=="")	{
			axis(2,at=tcs,tcl=pts[i],labels=abs(tcs),lwd=0.0,lwd.ticks=4/3,cex.axis=myr_size,las=2);
			} else	{
			axis(2,at=tcs,tcl=pts[i],labels=abs(tcs),lwd=0.0,lwd.ticks=4/3,cex.axis=myr_size,family=font,las=2);
			}
		}
	}

if (alt_back && colored!="backdrop")	for (b in first_period:last_period)	if (gtools::even(b))	rect(mnx,max(onset,time_scale_to_plot[b]),mxx,min(end,time_scale_to_plot[b+1]),col=alt_back_hue,border=alt_back_hue);

if (hues)	{
	if (colored=="base")	{
		for (b in first_period:last_period)
			rect(mnx+(2*dxx)-(2*exx),max(onset,time_scale_to_plot[b]),(mnx-2*exx),min(end,time_scale_to_plot[b+1]),col=as.character(strat_colors[b]),border=as.character(strat_colors[b]),lwd=0);
		} else {
		for (b in first_period:last_period)
			rect(mnx,max(onset,time_scale_to_plot[b]),mxx,min(end,time_scale_to_plot[b+1]),col=strat_colors[b],border=strat_colors[b],lwd=0);
		rect(mnx+exx,onset,mxx,end,col=makeTransparent("white",50),border=makeTransparent("white",50));
		}
	}

# put in lines separating stages
for (s in first_period:last_period)	segments(mnx+(2*dxx)-(2*exx),time_scale_to_plot[s],mnx-2*exx,time_scale_to_plot[s],lwd=0.5,col="gray25");
# put in decisive final line
if (!is.na(match(end,time_scale_to_plot)))	segments(end,mnx+(2*dxx)-(2*exx),end,mnx-2*exx);
segments(mnx+(2*dxx)-(2*exx),onset,mnx+(2*dxx)-(2*exx),end,lwd=2);
segments(mnx-2*exx,onset,mnx-2*exx,end,lwd=2);
...
if (use_strat_labels)	{
	...
	for (i in 1:(lst-1))		mid[i] <- (max(onset,time_scale_to_plot[i])+min(end,time_scale_to_plot[(i+1)]))/2;
	dummy <- vector(length=(lst-1));
	for (i in 1:(lst-1))		dummy[i] <- ((mnx-2*exx)+(mnx+2*dxx-2*exx))/2;
	#add something
	font_size <- min(2,3*thin_bin*ysize*strat_label_size);
#	print(font_size);
	if (font=="")	{
		text(dummy[first_period:last_period],mid[first_period:last_period],label=strat_names[first_period:last_period],cex=font_size,las=2,srt=90);
		} else	{
		text(dummy[first_period:last_period],mid[first_period:last_period],label=strat_names[first_period:last_period],cex=font_size,family=font,las=2,srt=90);
		}
	}
}

Phanerozoic_Timescale_Plot_Flexible_Ordinate_old <- function(onset=-541,end=0,time_scale_to_plot,mxx,mnx,use_strat_labels=TRUE,strat_names,strat_colors,plot_title="",abscissa="",ordinate="Ma",yearbreaks,ysize=6,xsize=4.285714285,hues=TRUE,colored="base",alt_back=FALSE,alt_back_hue="gray90",strat_label_size=1,stage_box_width=0.025)	{
# UPDATED 2016-12-31
#	Smaller updates 2018-08-31
...
mollusc_symbol <- function(x,y,abc,ord,size,whorls,W,style=c("snail","cephalopod"))	{
# x: x-coordinate
# y: y-coordinate
# abc: relative size of X-axis
# ord: relative size of Y-axis
...
# W: Raup's W.
if (lenght(style)>1)	style <- style[1];
...
# x, y: x & y coabscissa (center of symbol)
...
size_in_sky <- function(size,distance)	{
return((size/distance)*(180/pi));
}
...
#### Regression Lines ####
regression_line_to_plot <- function(max_x,min_x,max_y,min_y,line_slope,y_intercept,line_type=2,line_weight=2,line_color="black")	{
# get regression line within restricted plot
graph_intercept_y_bot <- y_intercept+(line_slope*min_x);
graph_intercept_y_top <- y_intercept+(line_slope*max_x);
graph_intercept_x_bot <- (min_y-y_intercept)/line_slope;
graph_intercept_x_top <- (max_y-y_intercept)/line_slope;

if (graph_intercept_y_bot>min_y)	{
	min_ply <- graph_intercept_y_bot;
	min_plx <- min_x;
	} else	{
	min_ply <- min_y;
	min_plx <- graph_intercept_x_bot;
	}
if (graph_intercept_y_top<max_y)	{
	max_ply <- graph_intercept_y_top;
	max_plx <- max_x;
	} else	{
	max_ply <- max_y;
	max_plx <- graph_intercept_x_top;
	}
segments(min_plx,min_ply,max_plx,max_ply,lwd=line_weight,lty=line_type,col=line_color);
}

regression_line_using_means_and_slope_to_plot <- function(max_x,min_x,max_y,min_y,line_slope,mean_x,mean_y,lty=2,lwd=2,line_color="black")	{
y_intercept_plot <- mean_y-line_slope*(mean_x-min_x);
x_intercept_plot <- mean_x-(mean_y-min_y)/line_slope;
if (y_intercept_plot>=min_y)	{
	x1 <- min_x;
	y1 <- y_intercept_plot
	} else	{
	x1 <- x_intercept_plot;
...
y_exit_plot <- mean_y+line_slope*(max_x-mean_x);
x_exit_plot <- mean_x+(max_y-mean_y)/line_slope;
if (y_exit_plot<=max_y)	{
	x2 <- max_x;
	y2 <- y_exit_plot
	} else	{
	x2 <- x_exit_plot;
...
segments(x1,y1,x2,y2,lty=lty,lwd=lwd,col=line_color);
}

#### Stylized Axes ####
#label_text <- "umbilical_diameter";
capitalize_each_word <- function(label_text)	{
label_text <- gsub("_"," ",label_text);
label_words <- strsplit(label_text," ")[[1]];
nwords <- length(label_words);
for (w in 1:nwords)	{
	if (!label_words[w] %in% c(lower_words,tolower(lower_words)))	{
		word_spelt <- strsplit(label_words[w],"")[[1]];
		word_spelt[1] <- toupper(word_spelt[1]);
		label_words[w] <- paste(word_spelt,collapse="");
		}
	}
return(paste(label_words,collapse=" "));
}

generate_log_scale <- function(max_no,min_no)	{
max_mag <- floor(log10(max_no));
max_no <- ceiling(max_no/10^max_mag)*10^max_mag;
min_mag <- floor(log10(min_no));
min_no <- floor(min_no/10^min_mag)*10^min_mag;
log_scale <- c();
for (mm in min_mag:max_mag)
	log_scale <- c(log_scale,(1:9)*(10^mm));
log_scale <- log_scale[log_scale>=min_no];
log_scale <- log_scale[log_scale<=max_no];
return(log_scale);
}

log_axes <- function(axe,min_ax,max_ax,increment,numbers,place_ticks=c(),linewd=4/3,orient,font_size=1,font="")	{
# increment: log distributed ticks
...
for (i in 1:ticks)	{
	l <- numbers[i];
  if (font=="")	{
  	axis(axe,at=log(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=linewd,las=orient,cex.axis=font_size);
  	} else	{
    axis(axe,at=log(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=linewd,las=orient,family=font,cex.axis=font_size);
#  	axis(axe,at=log10(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,family=font);
  	}
	}

if (is.null(place_ticks))	{
	for (i in 1:(ticks-1))	{
		l <- numbers[i]+increment;
		axis(axe,at=log(l),tcl=-.15,labels=NA,lwd=0,lwd.ticks=linewd);
		}
	} else	{
	place_ticks <- place_ticks[!place_ticks %in% numbers];
	for (i in 1:length(place_ticks))	{
		l <- place_ticks[i];
		axis(axe,at=log(l),tcl=-.15,labels=NA,lwd=0,lwd.ticks=linewd);
		}
	}
...
log10_axes <- function(axe,min_ax,max_ax,numbers,med_ticks=c(),min_ticks=c(),linewd=4/3,font_size=1,orient=1,axis_pos=NULL,coeff_pos=NULL,font="")	{
...
axis(axe,at=seq(min_ax,max_ax,by=(max_ax-min_ax)),tcl=0,labels=NA,lwd.ticks=NA, lwd=linewd, las=orient,pos=axis_pos);

mnn <- ceiling(min_ax);
mxn <- floor(max_ax);
if (mnn<mxn & length(med_ticks)==0 & length(min_ticks)==0)	{
	for (i in mnn:mxn)	{
		if (!is.null(axis_pos))	{
			axis(axe,at=i,tcl=-.3,labels=FALSE,lwd=0,lwd.ticks=linewd,pos=axis_pos);
			} else	{
	...
	for (i in 1:length(numbers))
		axis(axe,at=log10(numbers[i]),tcl=-.3,labels=FALSE,lwd=0,lwd.ticks=linewd,pos=axis_pos);
	}
for (i in 1:length(numbers))	{
	l <- numbers[i];
	if (font=="")	{
		if (is.null(coeff_pos))	{
			axis(axe,at=log10(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos);
			} else if (orient %in% c(1,3))	{
			axis(axe,at=log10(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos,padj=coeff_pos);
			} else if (orient %in% c(2,4))	{
			axis(axe,at=log10(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos,hadj=coeff_pos);
			}
		} else	{
		if (is.null(coeff_pos))	{
			axis(axe,at=log10(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,family=font,pos=axis_pos);
		} else if (orient %in% c(1,3))	{
			axis(axe,at=log10(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos,padj=coeff_pos,family=font);
			} else if (orient %in% c(2,4))	{
			axis(axe,at=log10(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos,hadj=0.75,family=font);
			}
		}
	}
...
if (max_ax>floor(max_ax) & mnn<mxn)	{
	add <- max(ticks)
...
if (mnn<mxn & length(med_ticks)==0 & length(min_ticks)==0)	{
	...
	axis(axe,at=log10(ticks),tcl=-.15,labels=FALSE,lwd=0,lwd.ticks=linewd,pos=axis_pos);
	} else if (length(med_ticks)>0)	{
	med_ticks <- med_ticks[!med_ticks %in% numbers];
	axis(axe,at=log10(med_ticks),tcl=-.15,labels=FALSE,lwd=0,lwd.ticks=linewd,pos=axis_pos);
	if (length(min_ticks)>0)	{
		min_ticks <- min_ticks[!min_ticks %in% c(med_ticks,numbers)];
		axis(axe,at=log10(min_ticks),tcl=-.15,labels=FALSE,lwd=0,lwd.ticks=linewd/2,pos=axis_pos);
		}
	} else if (length(min_ticks)>0)	{
	min_ticks <- min_ticks[!min_ticks %in% numbers];
	axis(axe,at=log10(min_ticks),tcl=-.15,labels=FALSE,lwd=0,lwd.ticks=linewd/2,pos=axis_pos);
	}
}

log10_axes_fancy <- function(axe,min_ax,max_ax,numbers,med_ticks=c(),min_ticks=c(),linewd=4/3,font_size=1,orient=1,axis_pos=NULL,coeff_pos=NULL,font="")	{
#log10_axes(axe=2,min_ax=mny,max_ax=mxy,numbers,linewd=1.5,orient=2)
# axe: 1 for x; 2 for y
# min_ax: minimum, already log10 transformed
# max_ax: maximum, already log10 transformed
...
print_numbers <- vector(length=length(numbers));
for (i in 1:length(numbers))	{
	l <- numbers[i];
	pow <- floor(log10(l));
	if (pow==log10(l))	{
		if (pow>=0)	{
			print_numbers[i] <- paste("10",common::supsc(pow),sep="");
			} else	{
			print_numbers[i] <- paste("10",common::supsc("-"),common::supsc(abs(pow)),sep="");
			}
		} else	{
		coef <- l*10^(0-pow)
		print_numbers[i] <- paste(coef,"×10",common::supsc(pow),sep="");
		}
	if (font=="")	{
		if (is.null(coeff_pos))	{
			axis(axe,at=log10(l),tcl=-.3,labels=print_numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos);
			} else if (orient %in% c(1,3))	{
			axis(axe,at=log10(l),tcl=-.3,labels=print_numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos,padj=coeff_pos);
			} else if (orient %in% c(2,4))	{
			axis(axe,at=log10(l),tcl=-.3,labels=print_numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos,hadj=coeff_pos);
			}
		} else	{
		if (is.null(coeff_pos))	{
			axis(axe,at=log10(l),tcl=-.3,labels=print_numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,family=font,pos=axis_pos);
		} else if (orient %in% c(1,3))	{
			axis(axe,at=log10(l),tcl=-.3,labels=print_numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos,padj=coeff_pos,family=font);
			} else if (orient %in% c(2,4))	{
			axis(axe,at=log10(l),tcl=-.3,labels=print_numbers[i],lwd=0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_pos,hadj=0.75,family=font);
			}
		}
	}
if (ceiling(min_ax)>min_ax)	{
...
log2_axes <- function(axe,min_ax,max_ax,numbers,numbers_med=NULL,linewd=4/3,font_size=1,orient=1,axis_pos=NULL,font="")	{
...
for (i in 1:ticks)	{
	...
	if (font=="")	{
		axis(axe,at=log2(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=linewd,cex.axis=font_size,las=orient);
		} else	{
		axis(axe,at=log2(l),tcl=-.3,labels=numbers[i],lwd=0,lwd.ticks=linewd,las=orient,cex.axis=font_size,family=font);
		}
	}
if (!is.null(numbers_med))
    axis(axe,at=log2(numbers_med),tcl=-.2,labels=FALSE,lwd=0,lwd.ticks=linewd);
}
...
tick_tock <- data.frame(maj_break=as.numeric(maj_break),med_break=as.numeric(med_break),min_break=as.numeric(min_break),stringsAsFactors = FALSE);
return(tick_tock);
}
...
tick_tock <- (10^fact10)*data.frame(maj_break=as.numeric(maj_break),med_break=as.numeric(med_break),min_break=as.numeric(min_break),stringsAsFactors = FALSE);
return(tick_tock);
}

set_log10_axis_breaks <- function(lin_max,lin_min,min_breaks=3,max_breaks=5)	{
numbers_to_return_med <- numbers_to_return <- c();
one_to_nine <- 1:9;
tens <- NULL;
if ((lin_max-lin_min)>20)	{
	a <- floor(log10(lin_min));
	z <- ceiling(log10(lin_max));
	tens <- 10^(a:z);
	numbers_to_return <- numbers_to_return_med <- c();
	for (i in a:z)	{
		numbers_to_return_med <- c(numbers_to_return_med,one_to_nine*(10^i));
		numbers_to_return <- c(numbers_to_return,c(1,5)*(10^i));
		}
	numbers_to_return <- numbers_to_return[numbers_to_return>=lin_min & numbers_to_return<=lin_max];
	numbers_to_return_med <- numbers_to_return_med[numbers_to_return_med>=lin_min & numbers_to_return_med<=lin_max];
	if (length(numbers_to_return)<min_breaks)	{
		numbers_to_return <- numbers_to_return_med;
		if (length(numbers_to_return)>1 & ((numbers_to_return[2]/10^floor(log10(numbers_to_return[2])))-(numbers_to_return[1]/10^floor(log10(numbers_to_return[2]))))==1)	{
			numbers_to_return_med <- c();
			aa <- floor(log10(numbers_to_return[1]))-1;
			numbers_to_return_med <- sort(numbers_to_return[1]-one_to_nine*10^aa);
			for (k in 1:length(numbers_to_return))	{
				aa <- floor(log10(numbers_to_return[k]))-1;
				numbers_to_return_med <- c(numbers_to_return_med,sort(numbers_to_return[k]+one_to_nine*10^aa));
				}
			numbers_to_return_med <- numbers_to_return_med[numbers_to_return_med>=lin_min & numbers_to_return_med<=lin_max];
			numbers_to_return_med <- numbers_to_return_med[!numbers_to_return_med %in% numbers_to_return];
			} else	{
			numbers_to_return_med <- c();
			}
		} else	{
	...
	} else if ((lin_max-lin_min)>=8)	{
	numbers_to_return <- (lin_min:lin_max)[((lin_min:lin_max) %% 2)==0];
	while (length(numbers_to_return)>max_breaks)	numbers_to_return <- numbers_to_return[((1:length(numbers_to_return)) %% 2)==1];
	numbers_to_return_med <- (lin_min:lin_max)[!(lin_min:lin_max) %in% numbers_to_return];
	} else if (length(log10(lin_min):log10(lin_max))>=min_breaks)	{
#	} else if (length(lin_min:lin_max)>=min_breaks)	{
	numbers_to_return <- 10^(ceiling(log10(lin_min)):floor(log10(lin_max)));
#	new_breaks <- one_to_nine/10^floor(log10(numbers_to_return[1]));
#	new_breaks <- one_to_nine/10^floor(log10(lin_min));
	if (length(numbers_to_return) >= 8*max_breaks)	{
		one_to_nine <- 5;
		}	else if (length(numbers_to_return) >= 3*max_breaks)	{
		one_to_nine <- 2^(1:3);
		}
	numbers_to_return_med <- one_to_nine*10^floor(log10(lin_min));
	for (ex in 1:length(numbers_to_return))
		numbers_to_return_med <- unique(c(numbers_to_return_med,one_to_nine*numbers_to_return[ex]));
#	numbers_to_return_med <- sort(numbers_to_return[1]-new_breaks);
	numbers_to_return_med <- numbers_to_return_med[numbers_to_return_med>=lin_min & numbers_to_return_med<=lin_max];
	numbers_to_return_med <- numbers_to_return_med[!numbers_to_return_med %in% numbers_to_return];
	if (length(numbers_to_return)>max_breaks)	{
		keep_one_in <- round(length(numbers_to_return)/max_breaks,0)
		keeper <- 1;
		if (lin_max>=1 & lin_min<=1)	{
			keeper <- match(1,numbers_to_return);
			if (is.na(keeper))	keeper <- 1+sum(numbers_to_return<1);
			keeper <- keeper %% max_breaks;
			}
		numbers_to_return <- numbers_to_return[(1:length(numbers_to_return)) %% keep_one_in==keeper]
		}
#	for (i in numbers_to_return)	numbers_to_return_med <- c(numbers_to_return_med,i+new_breaks);
#	numbers_to_return_med <- numbers_to_return_med[numbers_to_return_med>lin_min & numbers_to_return_med<lin_max];
#	numbers_to_return_med <- numbers_to_return_med[!numbers_to_return_med %in% numbers_to_return]
#	numbers_to_return <- numbers_to_return[numbers_to_return>=lin_min & numbers_to_return<=lin_max];
	} else if (floor(lin_min)>=1)	{
	numbers_to_return <- ceiling(lin_min):floor(lin_max);
	if (length(numbers_to_return)>=2)	{
			nn <- (numbers_to_return[2]-numbers_to_return[1])/2;
			numbers_to_return <- seq(numbers_to_return[1]-nn,numbers_to_return[2]+nn,by=nn);
			numbers_to_return <- numbers_to_return[numbers_to_return>=lin_min & numbers_to_return<=lin_max];
			new_breaks <- one_to_nine/10^ceiling(log10(numbers_to_return[1]));
			numbers_to_return_med <- sort(floor(numbers_to_return[1])-new_breaks);
			for (i in floor(min(numbers_to_return)):ceiling(max(numbers_to_return)))
				numbers_to_return_med <- c(numbers_to_return_med,i+new_breaks);
			numbers_to_return_med <- numbers_to_return_med[numbers_to_return_med>=lin_min & numbers_to_return_med<=lin_max];
			numbers_to_return_med <- numbers_to_return_med[!numbers_to_return_med %in% numbers_to_return];
			} else if (length(numbers_to_return)==1)	{
		numbers_to_return <- seq(lin_min,lin_max,by=(10^floor(log10(numbers_to_return)))/2);
		numbers_to_return_med <- seq(lin_min,lin_max,by=10^(floor(log10(numbers_to_return[1]))-1));
		}
	} else	{
	numbers_to_return <- 10^(floor(log10(lin_min)):ceiling(log10(lin_max)));
	numbers_to_return_med <- c();
	for (nr in 1:length(numbers_to_return))
		numbers_to_return_med <- c(numbers_to_return_med,one_to_nine*numbers_to_return[nr]);
	numbers_to_return_med <- numbers_to_return_med[!numbers_to_return_med %in% numbers_to_return];
	numbers_to_return_med <- numbers_to_return_med[numbers_to_return_med>=lin_min & numbers_to_return_med<=lin_max];
	numbers_to_return <- numbers_to_return[numbers_to_return>=lin_min & numbers_to_return<=lin_max];
	}
#& length(numbers_to_return_mid)>(max_breaks*10)
if (length(numbers_to_return)>max_breaks & !is.null(tens))	{
	if (length(tens)>min_breaks)	{
		numbers_to_return_med <- sort(c(numbers_to_return_med,numbers_to_return[!numbers_to_return %in% tens]))
		numbers_to_return <- numbers_to_return[numbers_to_return %in% tens];
		}
	}
while (length(numbers_to_return)>max_breaks)	{
	if ((numbers_to_return[1] %% 2)==0)	{
		numbers_to_return_med <- numbers_to_return[((1:length(numbers_to_return)) %% 2)==0];
		numbers_to_return <- numbers_to_return[((1:length(numbers_to_return)) %% 2)==1];
		} else	{
		numbers_to_return_med <- numbers_to_return[((1:length(numbers_to_return)) %% 2)==1];
		numbers_to_return <- numbers_to_return[((1:length(numbers_to_return)) %% 2)==0];
		}
	}
while (length(numbers_to_return_med)>(max_breaks*9))	{
	numbers_to_return_med <- sort(c(numbers_to_return_med,numbers_to_return));
	numbers_to_return_med <- numbers_to_return_med[((1:length(numbers_to_return_med)) %% 2)==0]
	numbers_to_return_med <- numbers_to_return_med[!numbers_to_return_med %in% numbers_to_return];
	}

log_numbers <- list(numbers_to_return,numbers_to_return_med);
names(log_numbers) <- c("main","mid");
return(log_numbers);
}
...
fitted_linear_axis <- function(axe,max_val,min_val,linewd=4/3,orient=1,decimals=TRUE,font="")	{
if (max_val<=10)	{
	...
	if (decimals==TRUE)	{
		...
		if (font=="")	{
			...
			axis(axe,at=seq(min_val,max_val,by=bs),tcl=-0.0,labels=TRUE,lwd=0.0,lwd.ticks=0.0,las=orient,family=font);
			}
		bs <- 0.1
		ticks <- seq(min_val,max_val,by=bs)[!seq(min_val,max_val,by=bs) %in% old_ticks]
		...
		if (font=="")	{
			...
			axis(axe,at=seq(min_val,max_val,by=1),tcl=-0.3,labels=TRUE,lwd=0.0,lwd.ticks=linewd,las=orient,family=font);
			}
		}
	} else if (max_val <=20)	{
	axis(axe,at=seq(min_val,max_val,by=(max_val-min_val)),tcl=0.0,labels=FALSE,lwd=linewd,las=orient)
	...
	if (font=="")	{
		...
		axis(axe,at=ticks,tcl=-0.30,labels=TRUE,lwd=0.0,lwd.ticks=linewd,las=orient,family=font);
		}
	old_ticks <- ticks
	bs <- 1
	ticks <- seq(min_val,max_val,by=bs)[!seq(min_val,max_val,by=bs) %in% old_ticks]
...
#main="";subtitle="";abscissa="";ordinate="";xsize=3;ysize=3;cexaxis=1;cexlab=1;cexmain=1;cexsub=1;xlab_off=2;ylab_off=2;font="";
specify_basic_plot <- function(mxx, mnx, mxy, mny, main="",subtitle="",abscissa="", ordinate="", xsize=3, ysize=3,cexaxis=1,cexlab=1,cexmain=1,cexsub=1,xlab_off=2,ylab_off=2,main_off=-1,font="")	{
par(pin=c(xsize,ysize));
if (font=="")	{
	plot(NA,type='n',axes=FALSE,xlab="",ylab="",xlim=c(mnx,mxx),ylim=c(mny,mxy),cex.axis=cexaxis,cex.lab=cexlab,cex.main=cexmain,cex.sub=cexsub);
	title(main=main, line=main_off, cex.main=cexmain);
	title(xlab=abscissa, line=xlab_off, cex.lab=cexlab);
	title(ylab=ordinate, line=ylab_off, cex.lab=cexlab);
	} else	{
	plot(NA,type='n',axes=FALSE,xlab="",ylab="",xlim=c(mnx,mxx),ylim=c(mny,mxy),cex.axis=cexaxis,cex.lab=cexlab,cex.main=cexmain,cex.sub=cexsub,family=font);
	title(main=main, line=main_off, cex.main=cexmain,family=font);
	title(xlab=abscissa, line=xlab_off, cex.lab=cexlab,family=font);
	title(ylab=ordinate, line=ylab_off, cex.lab=cexlab,family=font);
	}
}
...
# axis_label_pos: where to print numbers (or whatever) next to axis.
specified_axis <- function(axe,max_val,min_val,maj_break,med_break,min_break,linewd=4/3,font_size=1,orient=1,print_label=TRUE,axis_label_pos=NULL,font="")	{
#axis(axe,at=seq(min_val,max_val,by=max_val-min_val),tcl=-0.0,labels = FALSE,lwd=linewd,las=orient)
if ((min_val/maj_break)-floor(min_val/maj_break)<(10^-10))	{
	...
	mxv2 <- mxv1 <- med_break*ceiling(max_val/med_break);
	if (mxv1>max_val)	{
		mxv1 <- max(seq(mnv1,max_val,by=maj_break));
		mxv2 <- max(seq(mnv1,max_val,by=med_break));
		}
	}
strt <- min(min_val,mnv1);
endy <- max(max_val,mxv1); endy <- max_val;
axis(axe,at=seq(strt,endy,by=endy-strt),tcl=-0.0,labels = FALSE,lwd=linewd,las=orient);
ticks <- old_ticks <- seq(mnv1,mxv1,by=as.numeric(maj_break));
axis(axe,at=ticks,tcl=-0.30,labels=FALSE,lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size);
if (font=="")	{
	axis(axe,at=ticks,tcl=0,labels=print_label,lwd=0.0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_label_pos);#,yaxp=-1
	} else	{
	axis(axe,at=ticks,tcl=0,labels=print_label,lwd=0.0,lwd.ticks=0,las=orient,cex.axis=font_size,pos=axis_label_pos,family=font);#,yaxp=-1
	}
if (med_break!=0 & med_break!=maj_break)	{
	mnv2 <- (med_break*ceiling(min_val/med_break));
	ticks <- seq(mnv2,mxv2,by=med_break)[!seq(mnv2,max_val,by=med_break) %in% old_ticks];
	if (min_break!=0)	{
		tck_sz <- -0.20
		} else	tck_sz <- -0.15
...
if (min_break!=0 & min_break!=med_break)	{
	...
	# added 2023-12-10
	if (sum(ticks %in% old_ticks)==0)	{
		ticks-old_ticks[1]
		base_new_tick <- ticks[match(min(abs(ticks-old_ticks[1])),abs(ticks-old_ticks[1]))]
		base_old_tick <- old_ticks[match(min(abs(old_ticks-base_new_tick)),abs(old_ticks-base_new_tick))];
		ticks <- ticks-(base_new_tick-base_old_tick);
		}
	ticks <- ticks[!ticks %in% old_ticks];
	ticks <- ticks[ticks<=max_val & ticks>=min_val]
	if (med_break!=0)	{
		tck_sz <- -0.10
		} else	tck_sz <- -0.15
	axis(axe,at=ticks,tcl=tck_sz,labels=FALSE,lwd=0.0,lwd.ticks=linewd,las=orient);
...
#axis_label_pts=NULL
specified_axis_w_labels <- function(axe,max_val,min_val,maj_break,med_break,min_break,axis_labels,linewd=4/3,label_pos=c("tick","mid"),text_angle=NULL,text_pos=0,axis_pos=NULL,font_size=1,orient=1,print_label=TRUE,font="")	{
#axis(axe,at=seq(min_val,max_val,by=max_val-min_val),tcl=-0.0,labels = FALSE,lwd=linewd,las=orient)
if (text_pos==0 & label_pos=="mid" & axe %in% c(2,4))	text_pos <- -5;
if (med_break==0)	med_break <- min_break <- maj_break;
if ((min_val/maj_break)-floor(min_val/maj_break)<(10^-10))	{
	# this is a kluge necessitated by tiny rounding errors....
	mnv1 <- min_val;
	mxv1 <- max_val;
...
#	if (min(endy,max(axis_labels)+(label_span-1))>0)
#		mxv1 <- endy <- min(endy,max(axis_labels)+(label_span-1));
	} else if (label_pos=="mid")	{
	label_span <- 1;
	}

axis(axe,at=seq(strt,endy,by=endy-strt),tcl=-0.0,labels = FALSE,pos=axis_pos,lwd=linewd,las=orient);
#ticks <- old_ticks <- seq(mnv1,mxv1,by=maj_break);
ticks <- old_ticks <- seq(strt,endy,by=maj_break);
axis(axe,at=ticks,tcl=-0.30,labels=FALSE,lwd=0.0,lwd.ticks=linewd,las=orient,pos=axis_pos);
axis_span <- max_val-min_val;
if (label_pos=="mid" && is.numeric(axis_labels[1]))	{
	labels_ticks <- axis_labels-0.5*label_span;
	if (labels_ticks[1]<min_val & (1+max(labels_ticks))<max_val)
		labels_ticks <- axis_labels+0.5*label_span;
#	if (max_val<0 && sum(labels_ticks>0)>=(length(labels_ticks)-1))
#	  labels_ticks <- -labels_ticks;
...
#	axis(axe,at=labels_ticks,tcl=-0.30,labels=axis_labels,lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size);
	if (font=="")	{
		if (axe==1 || axe==3)	{
			axis(axe,at=labels_ticks[pass_one],tcl=-0.30,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,padj=text_pos,pos=axis_pos);
			axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,padj=text_pos,pos=axis_pos);
			} else	{
			axis(axe,at=labels_ticks[pass_one],tcl=-0.30,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,hadj=text_pos,pos=axis_pos);
			axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,hadj=text_pos,pos=axis_pos);
			}
#		axis(axe,at=labels_ticks,tcl=-0.30,labels=axis_labels,lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,hadj=hadj,padj=padj);
		} else	{
		if (axe==1 || axe==3)	{
			axis(axe,at=labels_ticks[pass_one],tcl=-0.30,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,padj=text_pos,family=font,pos=axis_pos);
			axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,padj=text_pos,family=font,pos=axis_pos);
			} else	{
			axis(axe,at=labels_ticks[pass_one],tcl=-0.30,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,hadj=text_pos,family=font,pos=axis_pos);
			axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,hadj=text_pos,family=font,pos=axis_pos);
			}
#		axis(axe,at=labels_ticks[pass_one],tcl=-0.30,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,family=font,hadj=hadj,padj=padj);
#		axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,family=font,hadj=hadj,padj=padj);
		}
	} else if (label_pos=="mid")	{
#	labels_ticks <- axis_labels-0.5*label_span;
	labels_ticks <- (1:length(axis_labels))-0.5;
	if (font=="")	{
		if (is.null(text_angle))	{
			axis(axe,at=labels_ticks,tcl=0,labels=axis_labels,lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,srt=text_angle,pos=axis_pos);
			} else	{
			for (i in 1:length(labels_ticks))
				text(x = labels_ticks[i], y = -1,axis_labels[i], srt = text_angle);
			}
#		axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size);
		...
		if (is.null(text_angle))	{
			for (lt in 1:length(labels_ticks))
				axis(axe,at=labels_ticks[lt],tcl=0,labels=axis_labels[lt],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,family=font,pos=axis_pos);
			} else	{
			for (i in 1:length(labels_ticks))
				text(x = labels_ticks[i], y = text_pos,axis_labels[i], srt = text_angle,family=font);
			}

#		axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,family=font);
		}
	} else	{
#	mnv <- maj_break*ceiling(min_val/maj_break);
#	mxv <- maj_break*ceiling(max_val/maj_break);
#	labels_ticks <- seq(mnv1,mxv1,by=maj_break);
	labels_ticks <- seq(strt,endy,by=maj_break);
	pass_one <- (1:length(axis_labels))[(1:length(axis_labels)) %% 2==1];
	pass_two <- (1:length(axis_labels))[(1:length(axis_labels)) %% 2==0];
	if (font=="")	{
		if (axe==1 || axe==3)	{
			axis(axe,at=labels_ticks[pass_one],tcl=0,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,padj=text_pos,pos=axis_pos);
			axis(axe,at=labels_ticks[pass_two],tcl=0,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,padj=text_pos,pos=axis_pos);
			} else	{
			axis(axe,at=labels_ticks[pass_one],tcl=0,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,hadj=text_pos,pos=axis_pos);
			axis(axe,at=labels_ticks[pass_two],tcl=0,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,hadj=text_pos,pos=axis_pos);
			}
		} else	{
		if (axe==1 || axe==3)	{
			axis(axe,at=labels_ticks[pass_one],tcl=0,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,padj=text_pos,family=font,pos=axis_pos);
			axis(axe,at=labels_ticks[pass_two],tcl=0,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,padj=text_pos,family=font,pos=axis_pos);
			} else	{
			axis(axe,at=labels_ticks[pass_one],tcl=0,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,hadj=text_pos,family=font,pos=axis_pos);
			axis(axe,at=labels_ticks[pass_two],tcl=0,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,hadj=text_pos,family=font,pos=axis_pos);
			}
		}
	}
...
if (med_break!=0 && med_break!=maj_break)	{
	...
	axis(axe,at=ticks,tcl=tck_sz,labels=FALSE,lwd=0.0,lwd.ticks=linewd,las=orient,pos=axis_pos)
	old_ticks <- sort(c(ticks,old_ticks))
	}
...
#axis_label_pts=NULL
specified_axis_w_controlled_labels <- function(axe,max_val,min_val,maj_break,med_break,min_break,controlled_labels,linewd=4/3,label_pos=c("tick","mid"),text_angle=NULL,text_pos=0,axis_pos=NULL,font_size=1,orient=1,print_label=TRUE,font="")	{
#axis(axe,at=seq(min_val,max_val,by=max_val-min_val),tcl=-0.0,labels = FALSE,lwd=linewd,las=orient)
if (text_pos==0 & label_pos=="mid")	text_pos <- -5;
if (med_break==0)	med_break <- min_break <- maj_break;
if ((min_val/maj_break)-floor(min_val/maj_break)<(10^-10))	{
	# this is a kluge necessitated by tiny rounding errors....
	mnv1 <- min_val;
...
axis(axe,at=seq(min_val,max_val,by=max_val-min_val),tcl=-0.0,labels = FALSE,pos=axis_pos,lwd=linewd,las=orient);
#ticks <- old_ticks <- seq(mnv1,mxv1,by=maj_break);
if (label_pos=="mid")	{
	ticks <- old_ticks <- controlled_labels;
	axis(axe,at=controlled_labels,tcl=-0.30,labels=FALSE,lwd=0.0,lwd.ticks=linewd,las=orient,pos=axis_pos);
	axis(axe,at=controlled_labels[1:(length(controlled_labels)-1)]+(controlled_labels[2]-controlled_labels[1])/2,tcl=-0.0,labels=names(controlled_labels),lwd=0.0,lwd.ticks=linewd,las=orient,pos=text_pos,family=font);
	} else	{
	ticks <- old_ticks <- controlled_labels;
	axis(axe,at=controlled_labels,tcl=-0.30,labels=FALSE,lwd=0.0,lwd.ticks=linewd,las=orient,pos=axis_pos);
	axis(axe,at=controlled_labels,tcl=0,labels=names(controlled_labels),lwd=0.0,lwd.ticks=0.0,las=orient,family=font,pos=text_pos);
	}
axis_span <- max_val-min_val;

if (med_break!=0 && med_break!=maj_break)	{
#	mnv2 <- (med_break*ceiling(min_val/med_break));
	ticks <- seq(controlled_labels[1],max(controlled_labels),by=med_break);
	if (min_val<controlled_labels[1])
		ticks <- sort(c(ticks,seq(min_val,controlled_labels[1],by=med_break)));
	if (max_val>max(controlled_labels))
		ticks <- sort(c(ticks,seq(max(controlled_labels),max_val,by=med_break)));
	ticks <- ticks[!ticks %in% old_ticks];
#	seq(mnv2,mxv1,by=med_break)[!seq(mnv2,max_val,by=med_break) %in% old_ticks]
	if (min_break!=0)	{
		tck_sz <- -0.20
		} else	tck_sz <- -0.15
	axis(axe,at=ticks,tcl=tck_sz,labels=FALSE,lwd=0.0,lwd.ticks=linewd,las=orient,pos=axis_pos)
...
if (min_break!=0 && !min_break %in% c(maj_break,med_break))	{
	ticks <- seq(controlled_labels[1],max(controlled_labels),by=min_break);
	if (min_val<controlled_labels[1])
		ticks <- sort(c(ticks,seq(min_val,controlled_labels[1],by=min_break)));
	if (max_val>max(controlled_labels))
		ticks <- sort(c(ticks,seq(max(controlled_labels),max_val,by=min_break)));
	ticks <- ticks[!ticks %in% old_ticks];
	if (med_break!=0)	{
		tck_sz <- -0.10
		} else	tck_sz <- -0.15
...
specified_axis_w_labels_less_old <- function(axe,max_val,min_val,maj_break,med_break,min_break,axis_labels,linewd=4/3,label_pos=c("tick","mid"),text_angle=NULL,text_pos=NULL,font_size=1,orient=1,print_label=TRUE,font="")	{
#axis(axe,at=seq(min_val,max_val,by=max_val-min_val),tcl=-0.0,labels = FALSE,lwd=linewd,las=orient)
if (med_break==0)	med_break <- min_break <- maj_break;
if ((min_val/maj_break)-floor(min_val/maj_break)<(10^-10))	{
	# this is a kluge necessitated by tiny rounding errors....
...
#	axis(axe,at=labels_ticks,tcl=-0.30,labels=axis_labels,lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size);
	if (font=="")	{
		...
		axis(axe,at=labels_ticks[pass_one],tcl=-0.30,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,family=font);
		axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=0.0,las=orient,cex.axis=font_size,family=font);
		}
	} else if (label_pos=="mid")	{
#	labels_ticks <- axis_labels-0.5*label_span;
	labels_ticks <- (1:length(axis_labels))-0.5;
	if (font=="")	{
		if (is.null(text_angle))	{
			axis(axe,at=labels_ticks,tcl=0,labels=axis_labels,lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,srt=text_angle);
			} else	{
			for (i in 1:length(labels_ticks))
				text(x = labels_ticks[i], y = -1,axis_labels[i], srt = text_angle);
			}
#		axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size);
		...
		if (is.null(text_angle))	{
			for (lt in 1:length(labels_ticks))
				axis(axe,at=labels_ticks[lt],tcl=0,labels=axis_labels[lt],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,family=font);
			} else	{
			for (i in 1:length(labels_ticks))
				text(x = labels_ticks[i], y = text_pos,axis_labels[i], srt = text_angle,family=font);
			}
...
#		axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,family=font);
	...
	mnv <- maj_break*ceiling(min_val/maj_break);
	mxv <- maj_break*ceiling(max_val/maj_break);
	labels_ticks <- seq(mnv,mxv,by=(mxv-mnv)/(length(axis_labels)-1));
	pass_one <- (1:length(axis_labels))[(1:length(axis_labels)) %% 2==1];
	pass_two <- (1:length(axis_labels))[(1:length(axis_labels)) %% 2==0];
	if (font=="")	{
		...
		axis(axe,at=labels_ticks[pass_one],tcl=-0.30,labels=axis_labels[pass_one],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,family=font);
		axis(axe,at=labels_ticks[pass_two],tcl=-0.30,labels=axis_labels[pass_two],lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,family=font);
		}
	}
...
specified_right_y_axis <- function(mxy1,mny1,max_val,min_val,maj_break,med_break,min_break,linewd=4/3,orient=2,ordinate2="",print_label=TRUE,font_size=1,font="")	{
...
if (print_label)	{
	if (font=="")	{
		axis(side=4,at=ticks,tcl=-0.30,labels=seq(mnv1,max_val,by=maj_break),lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size);
		} else	{
		axis(side=4,at=ticks,tcl=-0.30,labels=seq(mnv1,max_val,by=maj_break),lwd=0.0,lwd.ticks=linewd,las=orient,family=font,cex.axis=font_size);
		}
	} else	{
	  if (font=="")	{
		axis(side=4,at=ticks,tcl=-0.30,labels=print_label,lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size);
		} else	{
		axis(side=4,at=ticks,tcl=-0.30,labels=print_label,lwd=0.0,lwd.ticks=linewd,las=orient,family=font,cex.axis=font_size);
		}
	}
if (ordinate2!="")  {
  corners = par("usr") #Gets the four corners of plot area (x1, x2, y1, y2)
  par(xpd = TRUE) #Draw outside plot area
  xbreadth <- corners[2]-corners[1];
  if (font=="") {0
    text(x = corners[2]+xbreadth/8, y = mean(corners[3:4]),ordinate2, srt = 270);
 #    mtext(ordinate2,side=4,line=2);
    } else  {
    text(x = corners[2]+xbreadth/8, y = mean(corners[3:4]),ordinate2, srt = 270,family=font);
#    mtext(ordinate2,side=4,line=2,family=font);
    }
...
split_axes <- function(axe=2,mxy=1,mny=0,splits=NULL,rel_splits,axis_gaps,span_starts,span_ends,labelled_major_ticks_list,unlabelled_major_ticks_list,axis_label_list,font_size=1,linewd=4/3,orient=1,font=NULL)	{
if (is.null(splits))	splits <- length(rel_splits);
base <- mny;
for (s in 1:splits)	{
	top=base+rel_splits[s];
	this_span <- span_ends[s]-span_starts[s];
	these_ticks <- base+(rel_splits[s]*(labelled_major_ticks_list[[s]]-span_starts[s])/(span_ends[s]-span_starts[s]));
	other_ticks <- base+(rel_splits[s]*(unlabelled_major_ticks_list[[s]]-span_starts[s])/(span_ends[s]-span_starts[s]));
	other_ticks <- other_ticks[!other_ticks %in% these_ticks];
	axis(axe,at=c(base,top),tcl=-0.0,labels = FALSE,lwd=linewd,las=orient);
	for (tt in 1:length(these_ticks))
		axis(axe,at=these_ticks[tt],tcl=-0.3,labels = axis_label_list[[s]][tt],lwd=linewd,las=orient,cex.axis=font_size,family=font);
	ot <- 0;
	while (ot < length(other_ticks))	{
		ot <- ot+1;
		axis(axe,at=other_ticks[ot],tcl=-0.3,labels = FALSE,lwd=linewd,las=orient,cex.axis=font_size,family=font);
		}
#	labeled_ticks <- labelled_major_ticks_list[[s]]
	if (s<splits)	{
		slash1 <- top+(axis_gaps[s])/3;
		slash2 <- top+2*(axis_gaps[s])/3;
		axis(axe,at=slash1,tcl=-0.4,labels = FALSE,lwd=linewd,las=orient,cex.axis=font_size,family=font);
		axis(axe,at=slash1,tcl=0.4,labels = FALSE,lwd=linewd,las=orient,cex.axis=font_size,family=font);
		axis(axe,at=slash2,tcl=-0.4,labels = FALSE,lwd=linewd,las=orient,cex.axis=font_size,family=font);
		axis(axe,at=slash2,tcl=0.4,labels = FALSE,lwd=linewd,las=orient,cex.axis=font_size,family=font);

		base <- top+axis_gaps[s];
		}
	}
}
...
vector_tree <- ladderize_vector_tree(vector_tree);
...
if (length(sampled_ancestors) < (notu+nNode))	sampled_ancestors <- c(rep(0,notu),sampled_ancestors);
...
needed_edits <- hist(final_pos,breaks=0:max(final_pos),plot=FALSE)$counts
...
while (length(too_many_here)>0) {
	...
	while (tmh < length(too_many_here))	{
		...
		for (pp in 1:length(problems))
			these_nodes <- c(these_nodes,which(mtree==problems[pp],arr.ind = TRUE)[1]);	# get the nodes containing problem cases;
...
# modified 2020-05-11
# modified 2022-12-13 for E6
draw_calibrated_phylogeny <- function(vector_tree,finds,durations,phylo_axis,apomorphies,orientation="vertical",taxon_labels="",otu_cols,lazarus_col="gray50",plot_stratigraphy="no",branching_col="black",lineage_lwd=4,branching_lwd=2,new_plot=FALSE,height=4,width=6,taxon_cex=0.5,font="")	{
# working as of 2019-07-10
# draws phylogeny onto an already configured plot or makes a new one
# vector_tree: vector giving the node (htu) number to which each taxon or node is attached; -1 signifies the base of the tree.
#	finds: notu x max finds matrix giving dates of fossil finds
#	durations: first column gives onset of lineage, second column gives end of lineage
#	phylo_axis: vector giving where each lineage should be drawn 'across' tree
#	apomorphies: number of apomorphies for each branch
...
if (plot_stratigraphy=="ranges" || plot_stratigraphy=="range")	{
	strat_ranges <- data.frame(FAD=as.numeric(rep(0,notu)),LAD=as.numeric(rep(0,notu)),stringsAsFactors = FALSE);
	for (n in 1:notu)	{
		if (sum(finds[n,]!=0)>0)	{
	...
	strat_ranges <- durations[1:notu,];
	colnames(strat_ranges) <- c("FAD","LAD");
	strat_ranges$FAD <- strat_ranges$LAD;
	}
# get ancestral species that obviate "ghost taxon" nodes
node_ancestors <- accersi_poss_ancestors_for_nodes(vector_tree=vector_tree,FA=durations[,1],apos=apomorphies);

if (length(otu_cols)==1)	otu_cols <- rep(otu_cols,notu);
...
while (nn < nNode)	{
	...
	f1 <- mtree[nn,!mtree[nn,] %in% c(0,node_ancestors[n])];
	if (node_ancestors[n]!=0)	durations$end[n] <- durations$end[node_ancestors[n]];
	durations$end[n] <- max(c(durations$end[n],durations$onset[f1]));
#	durations$end[n] <- min(c(durations$end[n],durations$onset[f1]));

#		durations$end[sampled_ancestors[n]] <- durations$end[n];
	if (orientation=="horizontal" && gotcha[nn]==0)	{
		segments(durations$onset[n],phylo_axis[n],durations$end[n],phylo_axis[n],lwd=lineage_lwd,col=branching_col);
		if (node_ancestors[n]==0)	{
			segments(durations$onset[n],phylo_axis[n],durations$end[n],phylo_axis[n],lwd=0.75*lineage_lwd,col=lazarus_col);
			gotcha[n] <- 1;
			}
		} else if (gotcha[nn]==0)	{
	...
	for (f in 1:length(f1))	{
		if (orientation=="horizontal")	{
			segments(durations$onset[f1[f]]-adj_z,phylo_axis[f1[f]],durations$onset[f1[f]]-adj_z,phylo_axis[n],lwd=branching_lwd,col=branching_col);
			if (gotcha[f1[f]]==0)
				segments(durations$onset[f1[f]],phylo_axis[f1[f]],durations$end[f1[f]],phylo_axis[f1[f]],lwd=lineage_lwd,col=branching_col);
			} else	{
...
for (n in 1:notu)	{
	if (orientation=="horizontal")	{
		segments(strat_ranges$FAD[n],phylo_axis[n],strat_ranges$LAD[n],phylo_axis[n],lwd=lineage_lwd);
		segments(strat_ranges$FAD[n],phylo_axis[n],strat_ranges$LAD[n],phylo_axis[n],lwd=0.75*lineage_lwd,col=otu_cols[n]);
		if (taxon_labels[1]!="")	{
			if (font=="")	{
				text(strat_ranges$LAD[n],phylo_axis[n],taxon_labels[n],pos=4,cex=taxon_cex);
				} else	{
				text(strat_ranges$LAD[n],phylo_axis[n],taxon_labels[n],pos=4,family=font,cex=taxon_cex);
				}
			}
		} else	{
		segments(phylo_axis[n],strat_ranges$FAD[n],phylo_axis[n],strat_ranges$LAD[n],lwd=lineage_lwd);
		segments(phylo_axis[n],strat_ranges$FAD[n],phylo_axis[n],strat_ranges$LAD[n],lwd=0.75*lineage_lwd,col=otu_cols[n]);
		if (taxon_labels[1]!="")	{
			text(phylo_axis[n],strat_ranges$LAD[n],taxon_labels[n],pos=3,cex=taxon_cex);
			}
		}
#	n <- 1+n;
	}
...
draw_calibrated_phylogeny_vertical <- function(vtree,strat_ranges,durations,apos,oldest=NULL,youngest=NULL,taxon_labels="",otu_cols,lazarus_col="gray50",branching_col="black",font="")	{
...
sampled_ancestors <- accersi_poss_ancestors_for_nodes(vector_tree=vtree,FA=strat_ranges[,1],apos)

if (is.null(oldest))	oldest <- min(durations)
...
for (n in 1:notu)	{
	...
	if (taxon_labels[1]=="numbers" || taxon_labels[1]=="Numbers")	{
		if (font=="")	{
			...
			text(phy_x[n],strat_ranges[n,2],n,pos=3,family=font);
			}
		} else if (!is.na(taxon_labels[1]))	{
		if (font=="")	{
			text(phy_x[n]+x_adj,strat_ranges[n,2]+y_adj,taxon_labels[n],srt=90,pos=4);
			} else	{
			text(phy_x[n]+x_adj,strat_ranges[n,2]+y_adj,taxon_labels[n],srt=90,pos=4,family=font);
			}
		}
	}
...
#draw_calibrated_phylogeny_horizontal <- function(vtree,finds,durations,apomorphies,oldest=NA,youngest=NA,taxon_labels=NA,otu_cols,lazarus_col="gray50",branching_col="black",plot_stratigraphy="ranges",new_plot=FALSE,xsize=4,ysize=6)	{
draw_calibrated_phylogeny_horizontal <- function(vtree,finds,durations,apomorphies,taxon_labels=NA,otu_cols,lazarus_col="gray50",branching_col="black",plot_stratigraphy="ranges",new_plot=FALSE,xsize=4,ysize=6)	{
# draws phylogeny onto an already configured plot
	...
	if (!is.na(taxon_labels))	{
		if (taxon_labels[1]=="numbers" || taxon_labels[1]=="Numbers")	{
			if (font=="")	{
				...
				text(phy_y[n],durations[n,2],n,pos=3,family=font);
				}
			} else if (!is.na(taxon_labels[1]))	{
			if (font=="")	{
				text(phy_y[n]+x_adj,durations[n,2]+y_adj,taxon_labels[n],srt=90,pos=4);
				} else	{
				text(phy_y[n]+x_adj,durations[n,2]+y_adj,taxon_labels[n],srt=90,pos=4,family=font);
				}
			}
		}
	}
...
draw_calibrated_phylogeny_flex <- function(vtree,finds,durations,apos,orientation="vertical",oldest=NA,youngest=NA,taxon_labels=NA,otu_cols,lazarus_col="gray50",branching_col="black",plot_stratigraphy="ranges",new_plot=FALSE,xsize=4,ysize=6,font="")	{
# draws phylogeny onto an already configured plot
notu <- match(-1,vtree)-1;
durations <- -abs(durations);
finds <- -abs(finds);
...
if (new_plot)	{
	...
	if (orientation=="vertical")	{
		...
		plot(NA,type='n',axes=TRUE,main="",xlab="",ylab="",ylim=c(mnz,mxz),xlim=c(1,max(phy_z)));
		} else	{
		par(pin=c(max(ysize,xsize),min(ysize,xsize)));
		plot(NA,type='n',axes=FALSE,main="",xlab="",ylab="",xlim=c(mnz,mxz),ylim=c(1,max(phy_z)));
		plot(NA,type='n',axes=TRUE,main="",xlab="",ylab="",xlim=c(mnz,mxz),ylim=c(1,max(phy_z)));
		}
	}
	...
	if (!is.na(taxon_labels))	{
		if (taxon_labels[1]=="numbers" || taxon_labels[1]=="Numbers")	{
			if (font=="")	{
				...
				text(phy_z[n],durations[n,2],n,pos=3,family=font);
				}
			} else if (!is.na(taxon_labels[1]))	{
			if (font=="")	{
				text(phy_z[n]+x_adj,durations[n,2]+y_adj,taxon_labels[n],srt=90,pos=4);
				} else	{
				text(phy_z[n]+x_adj,durations[n,2]+y_adj,taxon_labels[n],srt=90,pos=4,family=font);
				}
			}
		}
	}
...
draw_calibrated_phylogeny_new <- function(vector_tree,finds,durations,phylo_axis,apomorphies,orientation="vertical",taxon_labels="",otu_cols,lazarus_col="gray50",plot_stratigraphy=FALSE,branching_col="black",lineage_lwd=4,lazarus_lwd=3,branching_lwd=2,lazarus_lty=1,taxon_label_cex=0.75,new_plot=FALSE,mxz=0,mnz=0,height=4,width=6,max_age=NULL)	{
# modified 2022-09-24
# working as of 2019-07-10
# draws phylogeny onto an already configured plot or makes a new one
# vtree: vector giving the node (htu) number to which each taxon or node is attached; -1 signifies the base of the tree.
# mxz: oldest age if this is an existing plot;
#	mnz: youngest age if this is an existing plot;
# plot_stratigraphy: default is "n"; if "range" or "ranges", it plots those
notu <- match(-1,vector_tree)-1;
colnames(durations) <- c("onset","end");
if (!is.data.frame(durations))	durations <- data.frame(onset=as.numeric(durations[,1],end=as.numeric(durations[,2])));
if (sum(durations$onset)>0 || sum(finds$FAD)>0)	finds <- -abs(finds);
if (plot_stratigraphy)	{
	strat_ranges <- data.frame(FAD=as.numeric(rep(0,notu)),LAD=as.numeric(rep(0,notu)),stringsAsFactors = FALSE);
	for (ntu in 1:notu)	{
		if (sum(finds[ntu,]!=0)>0)	{
			strat_ranges$FAD[ntu] <- min(finds[ntu,finds[ntu,]!=0]);
			strat_ranges$LAD[ntu] <- max(finds[ntu,finds[ntu,]!=0]);
			}
		}
	} else	strat_ranges <- finds;

if (sum(durations$onset-durations$end)<0) {
	fossil_ranges <- data.frame(fa=rowMins(finds),la=(rowMaxs(finds)));
	} else	{
	finds[finds==0] <- MAXNO;
	fossil_ranges <- data.frame(fa=-abs(rowMaxs(finds)),la=-abs(rowMins(finds)));
	finds[finds==MAXNO] <- 0;
	}

# get ancestral species that obviate "ghost taxon" nodes 2022-09-28 HERE
node_ancestor_otus <- accersi_poss_ancestors_for_nodes(vector_tree=vector_tree,FA=strat_ranges$FA,apos=apomorphies);
obs_nodes <- node_ancestor_otus[(notu+1):length(vector_tree)];

if (length(otu_cols)==1)	otu_cols <- rep(otu_cols,notu);

if (new_plot)	{
	mxz <- -abs(0.5*ceiling(min(abs(finds))/0.5));
	mxz <- -min(abs(fossil_ranges$la))
	if (is.null(max_age))	{
		mnz <- -abs(0.5*ceiling(max(abs(durations))/0.5));
		} else	{
		mnz <- -abs(0.5*ceiling(max_age)/0.5);
		}
	time_breaks <- as.numeric(set_axis_breaks_new(max_no=ceiling(max(abs(finds))),min_no = floor(min(abs(finds)))));
	maj_break <- max(time_breaks);
	med_break <- median(time_breaks);
	min_break <- min(time_breaks);
	if (taxon_labels[1]=="")	{
		exy <- 0;
		} else	{
		exy <- (mxz-mnz)/20
	...
	if (orientation=="vertical")	{
		par(pin=c(width,height));
		plot(NA,type='n',axes=FALSE,main="",xlab="",ylab="Time",ylim=c(mnz,mxz+exy),xlim=c(1,max(phylo_axis)));
		specified_axis(axe=2,max_val=mxz,min_val=mnz,maj_break,med_break,min_break,linewd=4/3,orient=2,print_label=TRUE);
		} else	{
		par(pin=c(height,width));
		plot(NA,type='n',axes=FALSE,main="",xlab="Time",ylab="",xlim=c(mnz,mxz+exy),ylim=c(1,max(phylo_axis)));
		specified_axis(axe=1,max_val=mxz,min_val=mnz,maj_break,med_break,min_break,linewd=4/3,orient=1,print_label=TRUE);
		}
	}
...
while (nn>1)	{
	...
	ntu <- notu+nn;
	obs_anc <- node_ancestor_otus[ntu];
	f1 <- mtree[nn,!mtree[nn,] %in% c(0,obs_anc)];
	# first, draw branches to descendants
	# add some visualization space if necessary!
	cladogenesis_ht <- durations$onset[f1];
	ch <- 1;
	while (ch < length(f1))	{
		ch <- ch+1;
		if ((cladogenesis_ht[ch-1]-cladogenesis_ht[ch])<extra_z)
			cladogenesis_ht[ch] <- cladogenesis_ht[ch]-extra_z;
		}
	for (f in 1:length(f1))	{
		if (orientation=="horizontal")	{
			segments(cladogenesis_ht[f]-adj_z,phylo_axis[f1[f]],cladogenesis_ht[f]-adj_z,phylo_axis[ntu],lwd=branching_lwd,col=branching_col);
			if (cladogenesis_ht[f]<durations$onset[f1[f]])
				segments(cladogenesis_ht[f]-adj_z,phylo_axis[f1[f]],durations$onset[f1[f]],phylo_axis[f1[f]],lwd=lazarus_lwd/3);
			} else	{
			segments(phylo_axis[f1[f]],cladogenesis_ht[f]-adj_z,phylo_axis[ntu],cladogenesis_ht[f]-adj_z,lwd=branching_lwd,col=branching_col);
			if (cladogenesis_ht[f]<durations$onset[f1[f]])
				segments(phylo_axis[f1[f]],cladogenesis_ht[f]-adj_z,phylo_axis[f1[f]],durations$onset[f1[f]],lwd=lazarus_lwd/3);
			}
		}
	# second, draw ancestral branch
	if(obs_anc!=0)	{
		lazarus_ub <- max(durations$end[ntu],finds$FAD[obs_anc]);
		lazarus_lb <- min(durations$onset[c(ntu,f1)]);
		} else	{
		lazarus_ub <- max(durations$onset[f1])
		lazarus_lb <- durations$onset[ntu];
		}
	if (orientation=="horizontal")	{
		segments(lazarus_lb,phylo_axis[ntu],lazarus_ub,phylo_axis[ntu],lwd=lazarus_lwd,col=branching_col);
		if (obs_anc==0)	{
#			segments(durations$onset[ntu],phylo_axis[ntu],lazarus_ub,				phylo_axis[ntu],lwd=lazarus_lwd,col=branching_col);
			segments(lazarus_lb,phylo_axis[ntu],lazarus_ub,phylo_axis[ntu],lwd=lazarus_lwd,col=lazarus_col);
			gotcha[ntu] <- 1;
			}
		} else	{
		segments(phylo_axis[ntu],lazarus_lb,phylo_axis[ntu],lazarus_ub,lwd=lazarus_lwd,col=branching_col,lty=lazarus_lty);
		if (obs_anc==0)	{
			segments(phylo_axis[ntu],lazarus_lb,phylo_axis[ntu],lazarus_ub,lwd=lazarus_lwd,col=lazarus_col,lty=lazarus_lty);
			gotcha[ntu] <- 1;
			}
		}
	# finally, draw range extensions up to base of daughter lines
	for (f in 1:length(f1))	{
		if (orientation=="horizontal")	{
#			segments(durations$onset[f1[f]]-adj_z,phylo_axis[f1[f]],durations$onset[f1[f]]-adj_z,phylo_axis[ntu],lwd=branching_lwd,col=branching_col);
			if (gotcha[f1[f]]==0)	segments(durations$onset[f1[f]],phylo_axis[f1[f]],durations$end[f1[f]],phylo_axis[f1[f]],lwd=lineage_lwd,col=branching_col);
			} else	{
#			segments(phylo_axis[f1[f]],durations$onset[f1[f]]-adj_z,phylo_axis[ntu],durations$onset[f1[f]]-adj_z,lwd=branching_lwd,col=branching_col);
			if (gotcha[f1[f]]==0)	segments(phylo_axis[f1[f]],durations$onset[f1[f]],phylo_axis[f1[f]],durations$end[f1[f]],lwd=lazarus_lwd,col=branching_col);
			}
		gotcha[f1[f]] <- 1;
		}
#	nn <- nn-1;
...
while (ntu < notu)	{
	ntu <- ntu+1;
	if (orientation=="horizontal")	{
		segments(fossil_ranges$fa[ntu],phylo_axis[ntu],fossil_ranges$la[ntu],phylo_axis[ntu],lwd=lineage_lwd);
		segments(fossil_ranges$fa[ntu],phylo_axis[ntu],fossil_ranges$la[ntu],phylo_axis[ntu],lwd=0.75*lineage_lwd,col=otu_cols[ntu]);
		text(fossil_ranges$la[ntu],phylo_axis[ntu],taxon_labels[ntu],pos=4);
		} else	{
		if (fossil_ranges$fa[ntu]!=fossil_ranges$la[ntu]) {
			segments(phylo_axis[ntu],fossil_ranges$fa[ntu],phylo_axis[ntu],fossil_ranges$la[ntu],lwd=lineage_lwd);
			segments(phylo_axis[ntu],fossil_ranges$fa[ntu],phylo_axis[ntu],fossil_ranges$la[ntu],lwd=0.75*lineage_lwd,col=otu_cols[ntu]);
			} else	{
			points(phylo_axis[ntu],fossil_ranges$fa[ntu],pch=22,bg=otu_cols[ntu],cex=0.75);
			}
		if (taxon_labels[1]!="")
			text(phylo_axis[ntu],fossil_ranges$la[ntu],taxon_labels[ntu],pos=3,cex=taxon_label_cex);
		}
#	n <- 1+n;
	}
}
...
# modified 2022-12-13 for E6
get_phylo_axis_from_newick_string <- function(newick_string,sampled_ancestors,root_low=TRUE)	{
## sampled_ancestors: vector of 0 & 1, with 1 = sampled ancestor
...
if (root_low)	{
	...
	end_node <- which(mat_tree==max_tax,arr.ind=TRUE)[,1];
...
obs_ancestors <- (1:notu)[sampled_ancestors==1];
nn <- nNodes+1;
while (nn > 1)	{
	nn <- nn-1;
	if(sum(mat_tree[nn,]>0)>2 && sum(mat_tree[nn,] %in% obs_ancestors)==1)	{
		this_anc <- mat_tree[nn,][mat_tree[nn,] %in% obs_ancestors];
		progeny <- mat_tree[nn,mat_tree[nn,]>0 & mat_tree[nn,]!=this_anc];
		clade_progeny <- progeny[progeny>notu];
		notu_progeny <- progeny[progeny<=notu];
		ttl_progeny <- venn_tree[nn,venn_tree[nn,]>0 & venn_tree[nn,]!=this_anc];
		if (min(otu_order[ttl_progeny])>otu_order[this_anc])	{
			if (length(notu_progeny)>0)	{
				flip <- min(otu_order[notu_progeny]);
				otu_order[notu_progeny][otu_order[notu_progeny]==flip] <- otu_order[this_anc];
				otu_order[this_anc] <- flip;
				} else	{
				nearest_prog <- min(otu_order[ttl_progeny]);
				dnd <- which(mat_tree==ttl_progeny[match(nearest_prog,otu_order[ttl_progeny])],arr.ind=TRUE)[,1];
				evictees <- venn_tree[dnd,venn_tree[dnd,]>0];
				flip <- max(otu_order[evictees]);
				otu_order[evictees] <- otu_order[evictees]-1;
				otu_order[this_anc] <- flip;
				}
			} else if (max(otu_order[ttl_progeny])<otu_order[this_anc])	{
			if (length(notu_progeny)>0)	{
				flip <- max(otu_order[notu_progeny]);
				otu_order[notu_progeny][otu_order[notu_progeny]==flip] <- otu_order[this_anc];
				otu_order[this_anc] <- flip;
				} else	{
				dnd <- which(mat_tree==ttl_progeny[match(max(otu_order[ttl_progeny]),otu_order[ttl_progeny])],arr.ind=TRUE)[,1];
				evictees <- venn_tree[dnd,venn_tree[dnd,]>0];
				flip <- min(otu_order[evictees]);
				otu_order[evictees] <- otu_order[evictees]+1;
				otu_order[this_anc] <- flip;
				}
			}
...
names(phylo_axis) <- 1:length(phylo_axis);
#test_axis <- phylo_axis;
...
#for (nd in nNodes:1)	{
	...
	old_phylo_axis <- phylo_axis; # for debugging
	if (sum(relv_ancestors[f1])>0)	{
		obs_anc <- f1[relv_ancestors[f1]==1][1];
		daughters <- f1[f1!=obs_anc];
		phylo_axis[htu] <- phylo_axis[obs_anc];
#		phylo_axis[htu] <- otu_order[obs_anc];
		if (length(daughters)>1 && (min(phylo_axis[daughters])>phylo_axis[htu] || max(phylo_axis[daughters])<phylo_axis[htu]))	{
			if (root_low)	{
				new_root <- floor(mean(phylo_axis[f1]));
				} else	{
				new_root <- ceiling(mean(phylo_axis[f1]));
				}
			retroth <- daughters[phylo_axis[daughters]<=new_root];
			for (rr in 1:length(retroth))	{
				if (retroth[rr]>notu)	{
					daughter_node <- retroth[rr]-rtaxa;
					all_descendants <- venn_tree[daughter_node,venn_tree[daughter_node,]>0];
					all_fruitful <- rtaxa+(1:nnodes)[venn_tree[,1] %in% all_descendants];
#					phylo_axis[c(all_descendants,all_fruitful)]
					if (min(phylo_axis[all_descendants])>phylo_axis[obs_anc])	{
						new_root <- max(phylo_axis[c(all_descendants,all_fruitful)]);
						phylo_axis[c(all_descendants,all_fruitful)] <- phylo_axis[c(all_descendants,all_fruitful)]-1;
						} else if (max(phylo_axis[all_descendants])<phylo_axis[obs_anc])	{
						new_root <- min(phylo_axis[all_descendants]);
						phylo_axis[c(all_descendants,all_fruitful)] <- phylo_axis[c(all_descendants,all_fruitful)]+1;
						} else if (phylo_axis[obs_anc]<max(phylo_axis[c(all_descendants,all_fruitful)]) && phylo_axis[obs_anc]>min(phylo_axis[c(all_descendants,all_fruitful)]))	{
						new_root <- max(phylo_axis[c(all_descendants,all_fruitful)]);
						phylo_axis[c(all_descendants,all_fruitful)][phylo_axis[c(all_descendants,all_fruitful)]>=phylo_axis[obs_anc]] <- phylo_axis[c(all_descendants,all_fruitful)][phylo_axis[c(all_descendants,all_fruitful)]>=phylo_axis[obs_anc]]-1;
						# it is somehow in the middle: we need to move it over
						}
					} else	{
					phylo_axis[retroth[rr]] <- phylo_axis[retroth[rr]]+axis_adj;
					}
				}
			phylo_axis[htu] <- phylo_axis[obs_anc] <- new_root;
			}
		} else	{
		phylo_axis[htu] <- mean(phylo_axis[mat_tree[nd,mat_tree[nd,]>0]])
		}
#	print(paste(nd,length(unique(phylo_axis[1:notu]))));
#	test_axis <- rbind(test_axis,phylo_axis);
#	print(test_axis);
#	print(length(unique(phylo_axis[1:notu])));
	}

return(phylo_axis);
}

# created 2022-12-13 for E6
untangle_stratigraphic_polytomies_in_phylo_axis <- function(vector_tree,phylo_axis,fas,sampled_ancestors)	{
notu <- min(vector_tree[vector_tree>0])-1;
mtree <- transform_vector_tree_to_matrix_tree(vector_tree);
venn_tree <- transform_vector_tree_to_venn_tree(vector_tree);
nnodes <- nrow(mtree);
sampled_nodes <- vector_tree[1:notu][sampled_ancestors==1]
tomies <- whatitomy(vector_tree);
names(tomies) <- 1:nnodes
tomies <- tomies[sampled_nodes-notu];
tomies <- tomies[tomies>3];
tt <- 0;
while (tt<length(tomies))	{
	tt <- tt+1;
	dodgy <- as.numeric(names(tomies)[tt]);
	htu <- dodgy+notu;
	f1 <- mtree[dodgy,mtree[dodgy,]>0];
	anc <- f1[f1<=notu][sampled_ancestors[f1[f1<=notu]]==1];
	f1 <- f1[f1!=anc];
	if (sum(phylo_axis[f1]>phylo_axis[htu])>1)	{
		f1b <- f1[phylo_axis[f1]>phylo_axis[htu]];
		# see if first branch out is furthest from node; if not, then we need to shuffle
		if(match(min(fas[f1b]),fas[f1b])!=match(max(phylo_axis[f1b]-phylo_axis[htu]),(phylo_axis[f1b]-phylo_axis[htu])))	{
			new_order <- order(-fas[f1b]);
			f1b <- f1b[order(-fas[f1b])];
			ttl_progeny <- 1;
			for (ff in 1:length(new_order))	{
				f2 <- f1b[ff];
#				adj <- (phylo_axis[htu]+ttl_progeny)-phylo_axis[f2];
				if (f2>notu)	{
					# get total progeny as well as daughter clades
					f2descs <- venn_tree[f2-notu,venn_tree[f2-notu,]>0];
					f2clades <- notu+(1:nnodes)[venn_tree[,1] %in% f2descs];
					shufty <- ttl_progeny+(phylo_axis[c(f2descs,f2clades)]-min(phylo_axis[c(f2descs,f2clades)]));
					phylo_axis[c(f2descs,f2clades)] <- shufty+phylo_axis[anc];
#					phylo_axis[f2descs] <- phylo_axis[f2descs]+adj;
#					phylo_axis[notu+(1:nnodes)[venn_tree[,1] %in% f2descs]] <- phylo_axis[notu+(1:nnodes)[venn_tree[,1] %in% f2descs]]+adj;
					ttl_progeny <- ttl_progeny+length(f2descs);
					} # finish adjusting granddaughters
					else	{
					phylo_axis[f2] <- phylo_axis[anc]+ttl_progeny;
					ttl_progeny <- ttl_progeny+1;
					}
				} # go through all descendants
			} # end case where we would have criss-crossed in the plot
		} # end case where we would have criss-crossed in the plot
	} # end search of possibly criss-crossing nodes
return(phylo_axis);
}

# modified 2021-09-15 for Crassatella fun.
# newick_string <- tree_output$newick
get_phylo_axis_from_newick_string_w_anagenesis_w_ladderize <- function(newick_string,sampled_ancestors,anagenetic_ancestors=c(),root_low=TRUE,ladderize=FALSE)	{
# sampled_ancestors: vector of 0 & 1, with 1 = sampled ancestor
if (ladderize)	newick_string <- ladderize_newick_string(newick_string);
...
while (an < length(nhtu))	{
	an <- an+1;
	observed_nodes[max(which(mat_tree==nhtu[an],arr.ind=TRUE)[,1])] <- nhtu[an];
	}
#anhtu <- (1:notu)[anagenetic_ancestors==1];	# anagenetic ancestors
nodal_species <- (1:notu)[sampled_ancestors>0];
names(nodal_species) <- names(sampled_ancestors)[nodal_species];
ana_species <- (1:notu)[anagenetic_ancestors>0];
names(ana_species) <- names(anagenetic_ancestors)[ana_species];
#ana_species <- (1:notu)[anagenetic_ancestors==1];  	# anagenetic ancestors
#names(ana_species) <- names(anagenetic_ancestors)[ana_species];
bud_species <- nodal_species[!nodal_species %in% ana_species];

# find anagenetic chains
ana_species_rem <- ana_species <- ana_species[order(vector_tree[as.numeric(ana_species)])];
anagenetic_chains <- list();
ac <- 0;
while (ac < length(ana_species))	{
	ac <- ac+1;
	if (ana_species[ac] %in% ana_species_rem)	{
		still_finding <- TRUE;
		nc <- ac;
		ana_chain <- as.numeric(ana_species[ac]);
		} else	{
		still_finding <- FALSE;
		}
	while (still_finding)	{
		# rewrite for polytomies some time.
		anny_node <- which(mat_tree==ana_chain[length(ana_chain)],arr.ind=T)[,1];
		mat_tree[anny_node,]
		# daughter clades;
		dc <- mat_tree[anny_node,][mat_tree[anny_node,]>notu];
		# daughter species;
		ds <- mat_tree[anny_node,][mat_tree[anny_node,]<=notu & mat_tree[anny_node,]!=ana_species[nc]];
		if (length(dc)==1)	{
			dcn <- dc-notu; # daughter node number
			# check if daughter node has an anagenetic ancestor
			if (sum(mat_tree[dcn,] %in% ana_species)==1)	{
				annie <- mat_tree[dcn,][mat_tree[dcn,] %in% ana_species];
				ana_chain <- c(ana_chain,annie)
				nc <- match(annie,ana_species);
				ana_species_rem <- ana_species_rem[!ana_species_rem %in% annie]
				still_finding <- TRUE;
				} else if (sum(mat_tree[dcn,] %in% observed_nodes)>0)	{
				# if daughter node has budding ancestor
				buddy <- mat_tree[dcn,][mat_tree[dcn,] %in% observed_nodes];
				ana_chain <- c(ana_chain,buddy);
				still_finding <- FALSE;
				} else {
				ana_chain <- c(ana_chain,dc);
				still_finding <- FALSE;
				}
			} else	{
#			ds <- ds[!ds %in% ana_species[ac]];
			ana_chain <- c(ana_chain,ds);
			if (sum(ds %in% ana_species)==0)
				still_finding <- FALSE;
			}
		}
	#ana_species_rem <- ana_species_rem[!ana_species_rem %in% ana_chain]
	anagenetic_chains <- unique(rlist::list.append(anagenetic_chains,ana_chain));
#	print(anagenetic_chains)
#	print(as.numeric(ana_species_rem));
	}

an <- 0;
while (an < length(anagenetic_chains))	{
	an <- an+1;
	ana_chain_spc <- anagenetic_chains[[an]];
	ana_chain_spc <- ana_chain_spc[ana_chain_spc<=notu];
	otu_order[ana_chain_spc] <- otu_order[ana_chain_spc[1]];
	}
otu_order <- match(otu_order,sort(unique(otu_order)));

# now, setup phylogenetic axis;
phylo_axis <- c(otu_order,rep(0,nNodes));
...
while (an < length(anagenetic_chains))	{
	an <- an+1;
	ana_chain_cld <- anagenetic_chains[[an]];
	ana_chain_cld <- ana_chain_cld[ana_chain_cld>notu];
	phylo_axis[ana_chain_cld] <- phylo_axis[anagenetic_chains[[an]][1]];
	}
...
while (nd>0) {
	htu <- nd+notu;
	f1 <- mat_tree[nd,];
	if (observed_nodes[nd]==0 & phylo_axis[htu]==0)	{
		phylo_axis[htu] <- mean(phylo_axis[f1]);
		} else if (observed_nodes[nd]==0 & phylo_axis[htu]>0)	{
#		print(nd);
#		print(phylo_axis[mat_tree[nd,mat_tree[nd,]>0]])
		} else	{
...
output <- list(phylo_axis,anagenetic_chains);
names(output) <- c("phylo_axis","anagenetic_chains");
return(output);
}

# matrix_tree <- strat_matrix_tree
get_phylo_axis_from_stratified_matrix_tree <- function(matrix_tree,sampled_ancestors,root_low=TRUE)	{
# sampled_ancestors: vector of 0 & 1, with 1 = sampled ancestor
nNodes <- nrow(matrix_tree);
notu <- max(matrix_tree)-nNodes;
venn_tree_st <- transform_matrix_tree_to_venn_tree(matrix_tree);
otu_order <- match((1:notu),venn_tree_st[1,])
vector_tree_st <- transform_matrix_tree_to_vector_tree(matrix_tree);
if (root_low)	{
	max_tax <- (1:notu)[match(max(otu_order),otu_order)];
	end_node <- which(matrix_tree==max_tax,arr.ind=TRUE)[,1];
	if (end_node < (nNodes/2))	{
		otu_order <- notu-otu_order
		}
	} else	{
...
phylo_axis <- c(otu_order,rep(0,nNodes));
relv_ancestors <- c(sampled_ancestors,rep(0,nNodes));
phylo_axis[vector_tree_st[(1:notu)[sampled_ancestors==1]]] <- phylo_axis[(1:notu)[sampled_ancestors==1]];
nd <- nNodes;
while (nd > 1)	{
#for (nd in nNodes:1)	{
	htu <- nd+notu;
	if (phylo_axis[htu]==0) {
		...
		poss_phy_ax <- mean(phylo_axis[f1]);
		if (!poss_phy_ax %in% phylo_axis) {
		  phylo_axis[htu] <- poss_phy_ax;
		  } else  {
		  new_axis <- min(phylo_axis[f1])
		  phylo_axis[phylo_axis>new_axis] <- phylo_axis[phylo_axis>new_axis]+1;
		  phylo_axis[htu] <- new_axis;
		  }
#		phylo_axis[htu] <- mean(phylo_axis[matrix_tree[nd,matrix_tree[nd,]>0]])
		}
	nd <- nd-1;
#	phylo_axis
	}
phylo_axis <- phylo_axis+(1-min(phylo_axis));
return(phylo_axis);
}

### This needs work!!!!
get_phylo_axis_from_vector_tree <- function(vector_tree,sampled_ancestors,anagenetic_ancestors=0,root_low=TRUE)	{
# sampled_ancestors: vector of 0 & 1, with 1 = sampled ancestor
notu <- match(-1,vector_tree)-1;
...
#	for (an in 1:length(anas))	{
	...
	while (an < length(anas))	{
		an <- an+1;
		acells <- which(mat_tree==anas[an],arr.ind = TRUE);
...
for (sp in 1:notu)	{
	i_o <- which(mat_tree==sp,arr.ind=TRUE);
	init_order[sp] <- xx;
	}
...
#### Add Image Files ####
add_png <- function(png_info, x = NULL,y = NULL,width = NULL,height = NULL,interpol = TRUE,x_cent=TRUE,y_cent=TRUE)	{
...
PIN <- par()$pin; # The current plot dimensions, (width, height), in inches
USR <- par()$usr; # A vector of the form c(x1, x2, y1, y2) giving the extremes of the user coordinates of the plotting region
jpg <- readJPEG(jpg_file, native=TRUE); # read theTfile
